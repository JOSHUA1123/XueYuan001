...
namespace SiteShow
{
    ...
    {
        //测试成绩记录的Id
        int trid = Common.Request.QueryString["trid"].Int32 ?? 0;
        XmlDocument resXml = new XmlDocument();
        protected override void InitPageTemplate(HttpContext context)
        {
            EntitiesInfo.TestResults result = Business.Do<ITestPaper>().ResultsSingle(trid);
            ...
            for (int i = 0; i < quesNodes.Count; i++)
            {
                ...
                if (tp == type)
                {
                    ...
                    for (int j = 0; j < qnode.Count; j++)
                    {
                        ...
                        {
                            double num = 0;
                            double.TryParse(qnode[j].Attributes["score"].Value, out num);
                            score += num;
                        }
                    }
                }
        ...
        {
            EntitiesInfo.Questions qus = (EntitiesInfo.Questions)obj[0];
            if (qus == null) return "";
            string ansStr = "";
            if (qus.Qus_Type == 1)
            {
