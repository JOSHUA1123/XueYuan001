...
namespace SiteShow.SOAP
{
    ...
    public class Organization : System.Web.Services.WebService
    {
        ...
        /// <returns></returns>
        [WebMethod]
        public EntitiesInfo.Organization Details()
        {
            //机构完整信息
           EntitiesInfo.Organization org = Business.Do<IOrganization>().OrganCurrent();
            string path =Common.Upload.Get["Org"].Virtual;
        ...
        {
            //所有机构完整信息
            EntitiesInfo.Organization[] org = Business.Do<IOrganization>().OrganAll(null, -1);
            string path = Common.Upload.Get["Org"].Virtual;
            foreach (EntitiesInfo.Organization o in org)
            {
