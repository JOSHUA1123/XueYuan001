...
namespace SiteShow.API
{
    ...
    public class SSO : IHttpHandler
    {
        ...
        string appid = Common.Request.QueryString["appid"].String;       //appid  
        string user = Common.Request.QueryString["user"].String;         //账号
        string name = Common.Request.QueryString["name"].UrlDecode;         //用户名称  
        string pw = Common.Request.QueryString["pw"].String;         //密码,md5加密
        string domain = Common.Request.QueryString["domain"].UrlDecode;  //来自请求的域名     
        string action = Common.Request.QueryString["action"].String.ToLower();     //动作，login登录，logout退出登录,verify校验密码是否正确,add新增用户
        string ret = Common.Request.QueryString["return"].String;     //返回类型,xml或json
        string goto_url = Common.Request.QueryString["goto"].UrlDecode;     //成功后的跳转地址
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            try
            {
                ...
                //接口是否存在或正确
                EntitiesInfo.SingleSignOn entity = Business.Do<ISSO>().GetSingle(appid);
                ...
                //通过验证，进入登录状态            
                EntitiesInfo.Accounts emp = Business.Do<IAccounts>().IsAccountsExist(user);
                if (emp == null)
                {
                ...
                else
                {
                    ...
                    switch (action)
                    {
                        ...
                        //验证密码
                        case "verify":
                            EntitiesInfo.Accounts acc = Business.Do<IAccounts>().AccountsLogin(emp.Ac_ID, pw, true);
