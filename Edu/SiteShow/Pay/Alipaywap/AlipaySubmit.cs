...
namespace Com.Alipaywap
{
    ...
    public class Submit
    {
        ...
        public Submit(int paiid)
        {
            _config = new Config(paiid);
            _private_key = _config.Private_key;
            _input_charset = _config.Input_charset.Trim().ToLower();
            _sign_type = _config.Sign_type.Trim().ToUpper();
        }
        public Submit(EntitiesInfo.PayInterface payInterface)
        {
            _config = new Config(payInterface);
            _private_key = _config.Private_key;
            _input_charset = _config.Input_charset.Trim().ToLower();
            _sign_type = _config.Sign_type.Trim().ToUpper();
        }
