...
namespace Site.Pay
{
    ...
    {
        //钱数
        private Decimal money = (Decimal)(Common.Request.Form["money"].Double ?? 0);
        //验证码
        string vpaycode = Common.Request.Form["vpaycode"].String; 
        //支付接口的id
        private int paiid = Common.Request.Form["paiid"].Int32 ?? 0;
        //是否校验验证码
        private bool isVerify = Common.Request.QueryString["isVerify"].Boolean ?? true;
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            {
                context.Response.Redirect(Common.Request.Page.AddPara(from, "err=1", "money=" + money, "paiid=" + paiid));
                return;
            }
            ////验证码不正确
            ...
            //}
            //支付接口的设置项
            EntitiesInfo.PayInterface pi = Business.Do<IPayInterface>().PaySingle(paiid);
            if (pi == null)
            {
                //设置项不存在
                context.Response.Redirect(Common.Request.Page.AddPara(from, "err=2", "money=" + money, "paiid=" + paiid));
                return;
            }
            else
            {
        ...
        /// 支付宝手机支付
        /// </summary>
        private void Alipaywap(EntitiesInfo.PayInterface pi, EntitiesInfo.MoneyAccount ma)
        {
            ////////////////////////////////////////////请求参数////////////////////////////////////////////
            //回调域
            string domain = "http://" + Common.Server.Domain + ":" + Common.Server.Port + "/";
        ...
        /// </summary>
        /// <param name="pi"></param>
        private void Alipayweb(EntitiesInfo.PayInterface pi, EntitiesInfo.MoneyAccount ma)
        {
            ////////////////////////////////////////////请求参数////////////////////////////////////////////
        ...
        /// <param name="pi"></param>
        /// <param name="ma"></param>
        private void Weixinpubpay(EntitiesInfo.PayInterface pi, EntitiesInfo.MoneyAccount ma)
        {
            string host = System.Web.HttpContext.Current.Request.Url.Host + ":" + Common.Server.Port + "/";
        ...
        /// <param name="pi"></param>
        /// <param name="ma"></param>
        private void WeixinNativepay(EntitiesInfo.PayInterface pi, EntitiesInfo.MoneyAccount ma)
        {
            string host = System.Web.HttpContext.Current.Request.Url.Host + ":" + Common.Server.Port + "/";
            if (!string.IsNullOrWhiteSpace(pi.Pai_Returl)) host = pi.Pai_Returl;
            if (!host.EndsWith("/")) host += "/";
        ...
        /// <param name="pi"></param>
        /// <param name="ma"></param>
        private void WeixinMiniProgramPay(EntitiesInfo.PayInterface pi, EntitiesInfo.MoneyAccount ma)
        {    
        ...
        /// <param name="pi"></param>
        /// <param name="ma"></param>
        private void WeixinH5Pay(EntitiesInfo.PayInterface pi, EntitiesInfo.MoneyAccount ma)
        {
        ...
        {
            //取图片验证码
            string imgCode = Common.Request.Cookies["vpaycode"].ParaValue;
            //取员工输入的验证码
            string userCode = new Common.Param.Method.ConvertToAnyValue(vpaycode).MD5;
            //验证
