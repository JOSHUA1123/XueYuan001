...
namespace SiteShow.Mobile
{
    ...
    public class MyFriends : BasePage
    {
        ...
        protected override void InitPageTemplate(HttpContext context)
        {
            ...
            {
                //如果未登录，则跳转到首页
                int sharekeyid = Common.Request.QueryString["sharekeyid"].Int32 ?? 0;
            ...
            //分润方案
            EntitiesInfo.ProfitSharing pstheme = Business.Do<IProfitSharing>().ThemeCurrent();
            this.Document.Variables.SetValue("pstheme", pstheme);
            //分润方案的分润项
            if (pstheme != null)
            {
                EntitiesInfo.ProfitSharing[] profits = Business.Do<IProfitSharing>().ProfitAll(pstheme.Ps_ID, true);
                this.Document.Variables.SetValue("profits", profits);
            }

            //分享二维码       
            string qrurl =Common.Request.Page.Authority+ "default.ashx?sharekeyid=" + accid;
            //各项配置           
            Common.CustomConfig config = CustomConfig.Load(this.Organ.Org_Config);   //自定义配置项           
            ...
            if (System.IO.File.Exists(centerImg))
                image = Common.QrcodeHepler.Encode(qrurl, 200, centerImg, color, "#fff");
            else
                image = Common.QrcodeHepler.Encode(qrurl, 200, color, "#fff");
            Common.Images.ImageTo.Overlay(image, this.Organ.Org_PlatformName, "down", 100,10,"","#000");
            //将image转为base64
