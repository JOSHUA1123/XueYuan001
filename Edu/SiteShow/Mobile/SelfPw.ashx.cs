...
namespace SiteShow.Mobile
{
    ...
    {
        //学员原密码，新密码
        protected string oldPw = Common.Request.Form["tbPw"].String;
        protected string newPw = Common.Request.Form["tbNewPw"].String;
        //状态代码
        ...
        {            
            if (!string.IsNullOrWhiteSpace(oldPw))
            {
                ...
                //通过验证，进入登录状态            
                EntitiesInfo.Accounts emp = Business.Do<IAccounts>().AccountsSingle(st.Ac_AccName, oldPw, this.Organ.Org_ID);
                if (emp == null)
                {
                    //密码不正确
