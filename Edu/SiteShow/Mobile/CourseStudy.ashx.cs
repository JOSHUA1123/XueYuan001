...
namespace SiteShow.Mobile
{
    ...
    public class CourseStudy : BasePage
    {
        ...
        protected override void InitPageTemplate(HttpContext context)
        {
            ...
            //当前章节
            EntitiesInfo.Outline ol = olid < 1 ?
            ...
            #region 章节输出
            // 当前课程的所有章节            
            EntitiesInfo.Outline[] outlines = Business.Do<IOutline>().OutlineAll(ol.Cou_ID, true);
            //课程章节列表
            ...
            ////视频
            //EntitiesInfo.Accessory video = SiteShow.CourseStudy.getVideo(ol.Ol_UID);
            //this.Document.Variables.SetValue("video", video);            
            //if (Extend.LoginState.Accounts.IsLogin)
            //{
            //    EntitiesInfo.LogForStudentStudy studyLog = Business.Do<IStudent>().LogForStudySingle(this.Account.Ac_ID, ol.Ol_ID);
            //    if (studyLog != null)
            //    {
            ...
            //}
            //附件
            List<EntitiesInfo.Accessory> access = Business.Do<IAccessory>().GetAll(ol.Ol_UID, "Course");
            if (access.Count > 0)
            {
