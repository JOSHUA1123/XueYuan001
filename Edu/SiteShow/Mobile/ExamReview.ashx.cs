...
namespace SiteShow.Mobile
{
    ...
    {
        //考试id
        int examid = Common.Request.QueryString["examid"].Int32 ?? 0;
        //考试成绩记录的Id
        int exrid = Common.Request.QueryString["id"].Int32 ?? 0;
        int accid = 0;
        XmlDocument resXml = new XmlDocument();
        protected override void InitPageTemplate(HttpContext context)
        {
            accid = Extend.LoginState.Accounts.UserID;
            EntitiesInfo.ExamResults result = null;
            if (exrid != 0)
            {
            ...
            EntitiesInfo.Examination exam = Business.Do<IExamination>().ExamSingle(result.Exam_ID);
            if (exam == null) return;
            //加载答题信息
            ...
            //试卷
            EntitiesInfo.TestPaper tp = Business.Do<ITestPaper>().PagerSingle((int)result.Tp_Id);
            this.Document.Variables.SetValue("pager", tp);
            //考生
            EntitiesInfo.Accounts st = Business.Do<IAccounts>().AccountsSingle((int)result.Ac_ID);
            this.Document.Variables.SetValue("st", st);
            
            //获取试题项
            EntitiesInfo.TestPaperItem[] tpi = getTpi(tp);
        ...
        /// </summary>
        /// <returns></returns>
        private EntitiesInfo.TestPaperItem[] getTpi(EntitiesInfo.TestPaper tp)
        {
            EntitiesInfo.TestPaperItem[] tpi = Business.Do<ITestPaper>().GetItemForAny(tp);
            List<TestPaperItem> list = new List<TestPaperItem>();
            XmlNodeList quesNodes = resXml.LastChild.ChildNodes;    //当前答案中的试题分类项
            foreach (EntitiesInfo.TestPaperItem t in tpi)
            {
        ...
        private string getTypeName(object[] id)
        {
            ...
            return Common.App.Get["QuesType"].Split(',')[type-1];
        }
        /// <summary>
        /// 该类型得分
        /// </summary>
        ...
        /// <returns></returns>
        private List<EntitiesInfo.Questions> getQues(object[] id)
        {
            int type = 0;
            if (id.Length > 0 && id[0] is int) type = Convert.ToInt32(id[0]);
            List<EntitiesInfo.Questions> list = new List<EntitiesInfo.Questions>();
            XmlNode root = resXml.LastChild;
            XmlNodeList quesNodes = root.ChildNodes;
            for (int i = 0; i < quesNodes.Count; i++)
            {
                ...
                if (tp == type)
                {
                    ...
                    for (int j = 0; j < qnode.Count; j++)
                    {
                        ...
                        EntitiesInfo.Questions ques = Business.Do<IQuestions>().QuesSingle(qid);
        ...
        /// <returns></returns>
        private List<EntitiesInfo.QuesAnswer> getItems(object[] p)
        {
            ...
            List<EntitiesInfo.QuesAnswer> list = new List<EntitiesInfo.QuesAnswer>();
        ...
        {
            EntitiesInfo.Questions qus = null;
            if (p.Length > 0) qus = (EntitiesInfo.Questions)p[0];
            if (qus == null) return "";
            string ansStr = "";
            if (qus.Qus_Type == 1)
            {
