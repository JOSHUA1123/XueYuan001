...
namespace SiteShow.Mobile
{
    ...
    {
        int colid = Common.Request.QueryString["colid"].Int32 ?? -1;  //栏目id
        protected override void InitPageTemplate(HttpContext context)
        {
            ...
            if (Request.ServerVariables["REQUEST_METHOD"] == "POST")
            {
                ...
                int sumcount = 0;
                EntitiesInfo.Article[] arts = Business.Do<IContents>().ArticlePager(Organ.Org_ID, colid, true, "", size, index, out sumcount);
                string json = "{\"size\":" + size + ",\"index\":" + index + ",\"sumcount\":" + sumcount + ",";
                json += "\"items\":[";
                for (int i = 0; i < arts.Length; i++)
                {
                    EntitiesInfo.Article art = arts[i];
                    //处理详情
                    art.Art_Details = "";
