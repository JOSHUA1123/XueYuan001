...
namespace SiteShow.Mobile
{
    ...
    {
        //当前考试的ID
        int examid = Common.Request.QueryString["id"].Int32 ?? 0;
        //考试的状态
        protected Examing_State state = null;
        protected override void InitPageTemplate(HttpContext context)
        {
            //删除考试成绩
            if (Common.Request.QueryString["again"].Int16 != null)
            {
                //删除考试成绩记录，以及清除缓存
            ...
            #region 当前考试信息
            //当前考试的场次
            EntitiesInfo.Examination exam = Business.Do<IExamination>().ExamSingle(examid);
            ...
            //当前考试的主题
            EntitiesInfo.Examination theme = Business.Do<IExamination>().ExamSingle(exam.Exam_UID);
            this.Document.SetValue("theme", theme);
            //当前试卷
            EntitiesInfo.TestPaper pager = Business.Do<ITestPaper>().PagerSingle(exam.Tp_Id);
            ...
            //答题记录
            EntitiesInfo.ExamResults exr = Business.Do<IExamination>().ResultSingleForCache(examid, exam.Tp_Id, Account.Ac_ID);
            //判断是否已经开始、是否已经结束
            ...
            this.Document.SetValue("startTime", Common.Server.getTime(startTime));
            this.Document.SetValue("overTime", Common.Server.getTime(overTime)); //转成时间戳
            ...
            //基本参数，uid,服务器端时间           
            this.Document.SetValue("servertime", Common.Server.getTime()); 
            this.Document.RegisterGlobalFunction(this.getSubjectPath);    //专业的上级专业等
            //试题类型
            this.Document.SetValue("types", Common.App.Get["QuesType"].String);
        }
        /// <summary>
        /// 获取当前专业的上级专业
        /// </summary>
