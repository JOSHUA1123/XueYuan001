...
namespace SiteShow.Mobile
{
    ...
    {
        //状态代码
        int state = Common.Request.QueryString["state"].Int16 ?? 0;
        protected override void InitPageTemplate(HttpContext context)
        {
            ...
            if (Request.ServerVariables["REQUEST_METHOD"] == "GET")
            {
                ...
                //来源
                string from = Common.Request.Form["from"].String;
            ...
            {
                //将账号信息写入
                EntitiesInfo.Accounts emp = Extend.LoginState.Accounts.CurrentUser;
                if (emp != null)
                {
        ...
        private void mobiregister_verify()
        {
            ...
            string userCode = Common.Request.Form["tbCode"].MD5;  //取输入的验证码
            string phone = Common.Request.Form["Ac_MobiTel1"].String;  //输入的手机号
            string sms = Common.Request.Form["tbSms"].MD5;  //输入的短信验证码          
            string rec = Common.Request.Form["rec"].String;  //推荐人的电话
            int recid = Common.Request.Form["recid"].Int32 ?? 0;  //推荐人的账户id
            //验证图片验证码
            if (imgCode != userCode)
            {
                Response.Write("{\"success\":\"-1\",\"state\":\"1\"}");   //图片验证码不正确
            ...
            //验证短信验证码
            bool isSmsCode = false;//true;      //是否短信验证；
            Common.CustomConfig config = CustomConfig.Load(this.Organ.Org_Config);
            isSmsCode = config["IsRegSms"].Value.Boolean ?? true;
            string smsCode = Common.Request.Cookies["reg_mobi_" + vname].ParaValue;
        ...
        /// <returns></returns>
        private EntitiesInfo.Accounts fillData(EntitiesInfo.Accounts acc)
        {
            //遍历实体属性
            ...
            for (int j = 0; j < properties.Length; j++)
            {
                ...
                string value = Common.Request.Form[pi.Name].String;
                if (pi.Name == "Ac_Pw") value = Common.Request.Form[pi.Name].MD5;
