...
namespace SiteShow.Mobile
{
    ...
    public class WeixinpubLogin : BasePage
    {
        ...
        protected override void InitPageTemplate(HttpContext context)
        {
            ...
            {
                string orgid = Common.Request.QueryString["state"].String;     //状码码是机构id                
                string openid = string.Empty;
                string access_token = getToken(out openid);
                //微信登录的回调域，（用途：如果回调域不是根域，则不再取当前机构二级域名)
                string returl = Business.Do<ISystemPara>()["WeixinpubReturl"].Value ?? Common.Server.MainName;
                ...
                if (returl.Equals(Common.Server.MainName, StringComparison.CurrentCultureIgnoreCase))
                {
                    EntitiesInfo.Organization org = getOrgan(-1);
                    returl = org.Org_TwoDomain + "." + Common.Server.MainName;
                }                
                string uri = "{0}/mobile/{1}?token={2}&openid={3}&orgid={4}";
                uri = string.Format(uri, returl, Common.Request.Page.FileName, access_token, openid, orgid);
            ...
            #region 具体操作代码
            string token = Common.Request.QueryString["token"].String;
        ...
        {
            string code = Common.Request.QueryString["code"].String;     //用户登录后，微信平台返回的代码
            ...
            try
            {
                string retjson = Common.Request.HttpGet(url);
        ...
        /// 获取当前机构
        /// </summary>
        protected EntitiesInfo.Organization getOrgan(int orgid)
        {
            EntitiesInfo.Organization org = null;
            if (orgid <= 0) orgid = Common.Request.QueryString["orgid"].Int32 ?? 0;
            if (orgid < 1) orgid = Common.Request.Cookies["ORGID"].Int32 ?? 0;
        ...
        private EntitiesInfo.Accounts getUserInfo(string access_token, string openid,out string unionid)
        {
            ...
            string retjson = Common.Request.HttpGet(userUrl);
        ...
        {
            string token = Common.Request.QueryString["token"].String;
            string openid = Common.Request.QueryString["openid"].String;
            this.Document.Variables.SetValue("openid", openid);
            this.Document.Variables.SetValue("token", token);
            //Common.Log.Write(string.Format("再次登录时，取token:{0},openid:{1}", token, openid));
            //设置主域，用于js跨根域
            ...
            EntitiesInfo.Organization org = getOrgan(-1);
            this.Document.Variables.SetValue("org", org);
            //Common.Log.Write(string.Format("再次登录时，机构{0}",org.Org_Name));
            //设置主域，用于js跨根域
            if (multi == 0 && !Common.Server.IsLocalIP)
                this.Document.Variables.SetValue("domain", Common.Server.MainName);
            this.Document.SetValue("domain2", org.Org_TwoDomain + "." + Common.Server.MainName);
            //获取帐户，如果已经注册，则直接实现登录
            string unionid = string.Empty;
            EntitiesInfo.Accounts acctm = getUserInfo(token, openid, out unionid);
            EntitiesInfo.Accounts acc = null;
        ...
        private void _directLogin()
        {
            string openid = Common.Request.QueryString["openid"].String;
            string token = Common.Request.QueryString["token"].String;
            int sex = Common.Request.Form["sex"].Int16 ?? 0;
            string name = Common.Request.Form["name"].String;
            string photo = Common.Request.Form["photo"].String;
            EntitiesInfo.Organization org = getOrgan(-1);           
            //创建新账户
            //获取微信登录账户的信息
            string unionid = string.Empty;
            EntitiesInfo.Accounts tmp = getUserInfo(token, openid, out unionid);
            if (tmp != null && !string.IsNullOrWhiteSpace(unionid))
            {
                ...
                Common.Request.LoadFile(photo, photoPath);
                tmp.Ac_Photo = unionid + ".jpg";
                //获取推荐人
                int recid = Common.Request.Cookies["sharekeyid"].Int32 ?? 0;
                EntitiesInfo.Accounts accRec = null;
        ...
        private void register1()
        {
            string token = Common.Request.QueryString["token"].String;
            string openid = Common.Request.QueryString["openid"].String;
            string mobi = Common.Request.Form["mobi"].String;    //手机号            
            int sex = Common.Request.Form["sex"].Int16 ?? 0;
            string name = Common.Request.Form["name"].String;
            string photo = Common.Request.Form["photo"].String;
            //验证手机号是否存在
            ...
            {
                EntitiesInfo.Accounts acc = Business.Do<IAccounts>().IsAccountsExist(-1, mobi, 1);
                if (acc != null)
                {
        ...
        private void register2()
        {
            ...
            string photo = Common.Request.Form["photo"].String;
            //短信验证码Cookie名称
            string smsName = Common.Request.Form["smsname"].String;
            string btnName = Common.Request.Form["smsbtn"].String;
            //验证图片验证码
            if (imgCode != userCode)
            {
            ...
            string smsCode = Common.Request.Cookies[smsName].ParaValue;
        ...
        private void bind1()
        {
            ...
            string openid = Common.Request.QueryString["openid"].String;
            string mobi = Common.Request.Form["mobi"].String;    //手机号
            string pw = Common.Request.Form["pw"].MD5;    //登录密码
            string vname = Common.Request.Form["vname"].String;
            string imgCode = Common.Request.Cookies[vname].ParaValue;    //取图片验证码
            string userCode = Common.Request.Form["vcode"].MD5;  //取输入的验证码
            int sex = Common.Request.Form["sex"].Int16 ?? 0;
