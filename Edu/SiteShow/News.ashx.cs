...
namespace SiteShow
{
    ...
    public class News : BasePage
    {
        ...
        {
            //获取所有栏目
            EntitiesInfo.Columns[] cols = Business.Do<IColumns>().ColumnCount(this.Organ.Org_ID, 0, "news", true, -1);
            ...
            if (newsTag != null)
            {
                ...
                int sum = 0;
                EntitiesInfo.Article[] news = Business.Do<IContents>().ArticlePager(Organ.Org_ID, columns, true, "", newsSize, index, out sum);
                foreach (EntitiesInfo.Article art in news)
                {
        ...
        /// <returns></returns>
        protected EntitiesInfo.Columns[] getColumns(object[] para)
        {
            int count = 0;
            if (para.Length > 0 && para[0] is int)
                int.TryParse(para[0].ToString(), out count);
            EntitiesInfo.Columns[] sts = Business.Do<IColumns>().ColumnCount(Organ.Org_ID, "News", true, count);
            return sts;
        }
    }
}