...
namespace SiteShow
{
    ...
    public class Training : BasePage
    {
        //各种参数
        //专业id,课程id,章节id
        int sbjid = Common.Request.QueryString["sbjid"].Int32 ?? 0;
        int couid = Common.Request.QueryString["couid"].Int32 ?? 0;
        int olid = Common.Request.QueryString["olid"].Int32 ?? 0;
        //题型，难度，题量
        int type = Common.Request.QueryString["type"].Int32 ?? 0;
        int diff = Common.Request.QueryString["diff"].Int32 ?? 0;
        int count = Common.Request.QueryString["count"].Int32 ?? 0;
        //试题的启始索引与当前取多少条记录
        protected string indexPara = Common.Request.QueryString["index"].String;
        ...
        private void _initLoyout()
        {
            ...
            if (sbjid > 0)
            {
                List<EntitiesInfo.Course> courses = Business.Do<ICourse>().CourseCount(Organ.Org_ID, sbjid, null, true, 0);
                this.Document.SetValue("courses", courses);
            }
            //章节     
            if (couid > 0)
            {
                EntitiesInfo.Outline[] outlines = Business.Do<IOutline>().OutlineCount(couid, 0, true, 0);
        ...
        {
            //当前试题
            EntitiesInfo.Questions qus = (EntitiesInfo.Questions)objs[0];
            string ansStr = "";
            if (qus.Qus_Type == 1)
            {
                //当前试题的答案
