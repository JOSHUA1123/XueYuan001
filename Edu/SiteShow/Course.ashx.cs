...
namespace SiteShow
{
    ...
    public class Course : BasePage
    {
        ...
        {            
            ...
            if (cou != null)
            {
                if ((Common.Request.Cookies["Course_" + cou.Cou_ID].Int32 ?? 0) == 0)
                {
            ...
            //课程公告
            EntitiesInfo.Guide[] guides = Business.Do<IGuide>().GuideCount(-1, cou.Cou_ID, -1, 20);
            this.Document.Variables.SetValue("guides", guides); 
            //当前课程的主讲老师
            EntitiesInfo.Teacher teacher = Business.Do<ITeacher>().TeacherSingle(cou.Th_ID);
            if (teacher != null)
            {
