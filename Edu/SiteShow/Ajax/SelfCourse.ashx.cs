...
namespace SiteShow.Ajax
{
    ...
    public class SelfCourse : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            if (context.Request.ServerVariables["REQUEST_METHOD"] == "POST")
            {
                ...
                else
                {
                    ...
                    {
                        case "buycou":
                            List<EntitiesInfo.Course> buyCou = Business.Do<ICourse>().CourseForStudent(stid, null, 1, false, -1);
                            context.Response.Write(_toJosn(buyCou, action, stid));
                            break;
                        case "overcou":
                            List<EntitiesInfo.Course> overCou = Business.Do<ICourse>().CourseForStudent(stid, null, 2, false, -1);
                            context.Response.Write(_toJosn(overCou, action, stid));
                            break;
                        case "trycou":
                            List<EntitiesInfo.Course> trycou = Business.Do<ICourse>().CourseForStudent(stid, null, -1, true, -1);
        ...
        /// 生成json
        /// </summary>
        protected string _toJosn(List<EntitiesInfo.Course> courses, string action,int stid)
        {
            ...
            for (int i = 0; i < courses.Count; i++)
            {
                ...
                //获取课程的购买信息
                Student_Course sc = Business.Do<ICourse>().StudentCourse(stid, c.Cou_ID);
                if (sc != null)
                {
