...
namespace SiteShow.Ajax
{
    ...
    public class ExamPager : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            //获取答题信息
            ExamResults exr = Business.Do<IExamination>().ResultSingleForCache(examid, tpid, stid);
        ...
        private string randomJson()
        {
            ...
            EntitiesInfo.TestPaper pager = Business.Do<ITestPaper>().PagerSingle(tpid);
            ...
            {
                //++++++++按题型输出
                TestPaperItem pi = (TestPaperItem)di.Key;   //试题类型                
                Questions[] ques = (Questions[])di.Value;   //当前类型的试题
            ...
            if (list.Count > 0)
            {
               ...
               Examination exam = Business.Do<IExamination>().ExamSingle(examid);
                if (exam != null)
                {
                    ...
                    Business.Do<IQuestions>().CacheAdd(list.ToArray<Questions>(), exam.Exam_Span + rn.Next(10), uid);
                }
            }
            return json;
        }
        ...
        /// <returns></returns>
        private string resultJson(ExamResults exr)
        {
        ...
        /// <param name="q"></param>
        /// <returns></returns>
        private string getQuesJson(Questions q)
        {
            //q = quesClear(q);
        ...
        /// <returns></returns>
        private string getAnserJson(Questions q, string json)
        {
            //当前试题的答案
           QuesAnswer[] ans = Business.Do<IQuestions>().QuestionsAnswer(q, null);
            string ansStr = "[";
