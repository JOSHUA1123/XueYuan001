...
namespace SiteShow.Ajax
{
    ...
    {
        //获取答题结果
        private string result = Common.Request.Form["result"].String;
        //答题结果的xml对象
        ...
        private string initData()
        {
            ...
            //
            TestResults exr = new TestResults();
            exr.Tp_Id = tpid;
            exr.Tp_Name = tpname;
            exr.Cou_ID = couid;
            exr.Sbj_ID = sbjid;
            ...
            var trsif=Business.Do<ITestPaper>().TestResultsInfo(exr.Ac_ID,exr.Tr_CrtTime);
            //返回得分与成绩id
            string json = "{\"score\":" + score + ",\"trid\":" + trsif.Tr_ID + ",\"tpid\":" + exr.Tp_Id + "}";
            return json;
