...
namespace SiteShow.Utility
{
    ...
    public class ExamFileUp : IHttpHandler
    {
        ...
        {
            //学员id，考试id
            int stid = Common.Request.QueryString["stid"].Int32 ?? 0;
            int examid = Common.Request.QueryString["examid"].Int32 ?? 0;
            int qid = Common.Request.QueryString["qid"].Int32 ?? 0; 
            ...
            if (files.Count > 0 && stid > 0)
            {
                ...
                string[] fileallow = Common.App.Get["ExamFileUp"].Split(',');
