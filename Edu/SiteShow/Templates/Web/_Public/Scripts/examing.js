...
function setQuestionLayout4(qitem, typeIndex, indexHanzi) {
    ...
    for (var i = 0; i < qitem.ques.length; i++) {
        ...
        html += "<div style='display: none' class='fileupBox' qid='" + q.Qus_ID + "'><span class='upfileArea'> 上传附件：<input type='file' class='fileinput' id='fileup_" + q.Qus_ID + "' qid='" + q.Qus_ID + "' name='file' />";
...
function setResultState(result) {
    ...
    for (var tm in ques) {
        ...
        if (q.type == 5) {
            ...
            //    $(this).val(arr[index]);
            //});
            $("#pagerArea dl dd[qid=" + q.id + "] .textbox").val(q.ans);
        }
    }
    //计算完成的题数
    $("#CompleteNumber").text($("#cardBox dd[class]").size());
