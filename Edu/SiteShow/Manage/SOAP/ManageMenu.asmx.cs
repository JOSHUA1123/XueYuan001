...
namespace SiteShow.Manage.SOAP
{
    ...
    public class ManageMenu : System.Web.Services.WebService
    {
        ...
        public EntitiesInfo.ManageMenu[] Roots(string functype)
        {
            //获取菜单树的列表，默认取显示状态的
            return Business.Do<IManageMenu>().GetRoot(functype, true);
        }
        [WebMethod]
        public EntitiesInfo.ManageMenu Root(int id)
        {
            //获取单个菜单树的对象
        ...
        [WebMethod]
        //用于前台显示的系统菜单
        public EntitiesInfo.ManageMenu[] DropMenu()
        {
            //获取菜单树的列表
        ...
        [WebMethod]
        //用于前台显示的功能菜单
        public EntitiesInfo.ManageMenu[] FuncMenu(int rootId)
        {
            //获取菜单树的所有菜单项
        ...
        [WebMethod]
        //获取所有可显示的栏目
        public EntitiesInfo.ManageMenu Menu(int id)
        {
            return Business.Do<IManageMenu>().GetSingle(id);
        }
        [WebMethod]
        ...
        {
            EntitiesInfo.ManageMenu nc = Business.Do<IManageMenu>().GetSingle(id);
        ...
        /// <returns></returns>
        [WebMethod]
        public EntitiesInfo.ManageMenu[] Tree(int rootId, string result)
        {
            int tmrootid = 0;
        ...
        /// <returns></returns>
        [WebMethod]
        public EntitiesInfo.ManageMenu[] UpdateTree(int rootId, string nodexml)
        {
            try
            {
        ...
        /// <returns></returns>
        [WebMethod]
        public EntitiesInfo.ManageMenu[] DelTree(int rootId, string nodexml)
        {
        ...
        {
            EntitiesInfo.ManageMenu[] mm = Business.Do<IManageMenu>().GetAll(rootid, null, null, type);
        ...
        private void _updateTreeNode(string nodexml)
        {
            ...
            //当前对象
            EntitiesInfo.ManageMenu mm = null;
            if (type == "edit" || type == "root") mm = Business.Do<IManageMenu>().GetSingle(id);
            if (type == "add") mm = new EntitiesInfo.ManageMenu();
        ...
        protected string btnPurTree(string type)
        {
            ...
            //生成左侧标题部分，也就是一级菜单
            string tm = "";
            foreach (EntitiesInfo.ManageMenu m in _allMM)
            {
                if (m.MM_PatId != 0) m.MM_IsShow = true;
            }
            Extend.MenuNode top = new Extend.MenuNode(null, _allMM);
            //生成每个菜单树的外框
            foreach (EntitiesInfo.ManageMenu m in top.Childs)
            {
        ...
        private string _PurBuidTree(EntitiesInfo.ManageMenu m)
        {
            //当前节点对象
            Extend.MenuNode n = new Extend.MenuNode(m, _allMM);
            //开始生成
            string temp = "";
        ...
        /// <param name="m"></param>
        /// <returns></returns>
        private string _PurBuildNode(EntitiesInfo.ManageMenu m)
        {
            string temp = "";
        ...
        private string _PurNodeIco(EntitiesInfo.ManageMenu m)
        {
            string temp = "";
            Extend.MenuNode n = new Extend.MenuNode(m, _allMM);
            //如果是根节点
        ...
        /// <returns></returns>
        private string _PurNodeLine(EntitiesInfo.ManageMenu m, int topid)
        {
            string temp = "";
            Extend.MenuNode mn = new Extend.MenuNode(m, _allMM);
            //当前菜单项的上级菜单项
            Extend.MenuNode p = new Extend.MenuNode(mn.Parent, _allMM);
            while (p.Item.MM_Id != topid)
            {
                ...
                //temp = empty + temp;
                p = new Extend.MenuNode(p.Parent, _allMM);
            }
