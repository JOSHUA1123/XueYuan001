...
namespace SiteShow.Manage.SOAP
{
    ...
    public class Columns : System.Web.Services.WebService
    {
        ...
        [WebMethod]
        //获取所有可显示的栏目
        public EntitiesInfo.Columns[] All()
        {
            //所属机构的所有课程
        ...
        [WebMethod]
        //获取单个栏目信息
        public EntitiesInfo.Columns Column(int id)
        {
        ...
        {
            EntitiesInfo.Columns nc = Business.Do<IColumns>().Single(id);
        ...
        private string _getTreeHtml()
        {
            //所属机构的所有课程
            EntitiesInfo.Organization org = Business.Do<IOrganization>().OrganCurrent();
            EntitiesInfo.Columns[] ncs = Business.Do<IColumns>().All(org.Org_ID, null);
        ...
        private void _Add(string result)
        {
            ...
            try
            {
                ...
                XmlNode node = resXml.SelectSingleNode("node");
                int id = Convert.ToInt32(((XmlElement)node).Attributes["id"].Value);
                EntitiesInfo.Columns nc = new EntitiesInfo.Columns();
                nc.Col_Name = ((XmlElement)node.SelectSingleNode("name")).InnerText;
                nc.Col_ByName = ((XmlElement)node.SelectSingleNode("byname")).InnerText;
                //父id
                int pid = 0;
