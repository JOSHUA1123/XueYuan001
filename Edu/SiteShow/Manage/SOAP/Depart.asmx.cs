...
namespace SiteShow.Manage.SOAP
{
    ...
    public class Depart : System.Web.Services.WebService
    {
        ...
        [WebMethod]
        //获取所有可显示的院系
        public EntitiesInfo.Depart[] Departs()
        {
        ...
        [WebMethod]
        //设置院系顺序，并返回共显示的所有院系
        public EntitiesInfo.Depart[] DepartOrder(string result)
        {
            this._SaveOrder(Server.UrlDecode(result));
            int orgid = Extend.LoginState.Admin.CurrentUser.Org_ID;
            EntitiesInfo.Depart[] deps= Business.Do<IDepart>().GetAll(orgid);
            return deps;
        }
        [WebMethod]
        //修改院系，并返回共显示的所有院系
        public EntitiesInfo.Depart[] DepartUpdate(string result)
        {
        ...
        [WebMethod]
        //添加院系，并返回共显示的所有院系
        public EntitiesInfo.Depart[] DepartAdd(string result)
        {
        ...
        [WebMethod]
        //添加院系，并返回共显示的所有院系
        public EntitiesInfo.Depart[] DepartDel(string result)
        {
        ...
        private void _Update(string result)
        {
            ...
            try
            {
                ...
                EntitiesInfo.Depart dep = Business.Do<IDepart>().GetSingle(id);
                if (dep == null) return;
        ...
        private void _Add(string result)
        {
            ...
            try
            {
                ...
                XmlNode node = resXml.SelectSingleNode("node");
                int id = Convert.ToInt32(((XmlElement)node).Attributes["id"].Value);
                EntitiesInfo.Depart dep = new EntitiesInfo.Depart();
                dep.Dep_CnName = ((XmlElement)node.SelectSingleNode("name")).InnerText;
