...
namespace SiteShow.Teacher
{
    ...
    public class Register : BasePage
    {
        ...
        private void mobivcode_verify()
        {
            ...
            //验证手机号是否存在
            EntitiesInfo.Accounts curr = Extend.LoginState.Accounts.CurrentUser;
            if (curr == null)
            {
        ...
        private void mobiregister_verify()
        {
            ...
            string name = Common.Request.Form["name"].String;     //姓名
            string email = Common.Request.Form["email"].String;     //邮箱
            string qq = Common.Request.Form["qq"].String;  //qq
            string idcard = Common.Request.Form["idcard"].String;  //身份证号
            string intro = Common.Request.Form["intro"].Text;  //简介
            //验证图片验证码
            if (imgCode != userCode)
            {
                Response.Write("{\"success\":\"-1\",\"state\":\"1\"}");   //图片验证码不正确
            ...
            {                
                EntitiesInfo.Accounts tmp = Extend.LoginState.Accounts.CurrentUser;
                tmp.Ac_IsTeacher = true;    //当前账户有了教师身份
                tmp.Ac_MobiTel2 = phone;
                Business.Do<IAccounts>().AccountsSave(tmp);
                //创建教师账户
                EntitiesInfo.Teacher th = Business.Do<IAccounts>().GetTeacher(curr.Ac_ID, null);
