...
namespace SiteShow
{
    ...
    public class Live : BasePage
    {
        ...
        {
            //正在直播的章节
            List<EntitiesInfo.Outline> lives = Business.Do<IOutline>().OutlineLiving(Organ.Org_ID, 10);
            if (lives != null)
            {
            ...
            if (couTag != null)
            {
                ...
                int sum = 0;
                List<EntitiesInfo.Course> cour = Business.Do<ICourse>().CoursePager(Organ.Org_ID, "", true, true, string.Empty, "rec", size, index, out sum);
