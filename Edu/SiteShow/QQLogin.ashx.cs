...
namespace SiteShow
{
    ...
    public class QQLogin : BasePage
    {
        ...
        protected override void InitPageTemplate(HttpContext context)
        {
            ...
            string code = Common.Request.QueryString["code"].String;     //验证用的code，只用一次即失效
            if (Request.ServerVariables["REQUEST_METHOD"] == "GET" && !string.IsNullOrWhiteSpace(code))
            {
                string orgid = Common.Request.QueryString["state"].String;     //机构id
                ...
                if (Common.Browser.IsMobile) uri = "/Mobile" + uri;
            ...
            #region 具体操作代码
            string access_token = Common.Request.QueryString["token"].String;
            ...
            if (Request.ServerVariables["REQUEST_METHOD"] == "GET" && !string.IsNullOrWhiteSpace(access_token))
            {
                ...
                if (multi == 0 && !Common.Server.IsLocalIP) this.Document.Variables.SetValue("domain", Common.Server.MainName);
                //QQ回调域
                string returl = Business.Do<ISystemPara>()["QQReturl"].Value ?? Common.Server.MainName;
                this.Document.SetValue("QQReturl", returl);
                //当前机构
                EntitiesInfo.Organization org = getOrgan();
                this.Document.SetValue("domain2", getOrganDomain(org));
                //获取帐户，如果已经注册，则直接实现登录
                EntitiesInfo.Accounts acc = _ExistAcc(openid);
                if (acc == null)
                {                    
                    //账户不存在，以下用于注册
        ...
        /// 获取当前机构
        /// </summary>
        protected EntitiesInfo.Organization getOrgan()
        {
            EntitiesInfo.Organization org = null;
            int orgid = Common.Request.QueryString["orgid"].Int32 ?? 0;
        ...
        /// <returns></returns>
        protected string getOrganDomain(EntitiesInfo.Organization org)
        {
            string root = Common.Server.MainName;
        ...
        {
            string code = Common.Request.QueryString["code"].String;     //用户登录后，qq平台返回的代码
        ...
        private EntitiesInfo.Accounts getUserInfo(string access_token, string openid)
        {
            ...
            //解析QQ账户信息
            EntitiesInfo.Accounts acc = new EntitiesInfo.Accounts();
            try
            {
                string infoJson = Common.Request.HttpGet(userUrl);
                ...
                if (ret == "0")
                {
                    ...
                    acc.Org_ID = Common.Request.QueryString["orgid"].Int32 ?? 0; //所属机构
                }
                else
                {
        ...
        /// </summary>
        /// <param name="openid"></param>
        private EntitiesInfo.Accounts _ExistAcc(string openid)
        {
            //获取帐户，如果已经注册，则直接实现登录
            EntitiesInfo.Accounts acc = Business.Do<IAccounts>().Account4QQ(openid);
            if (acc != null)
            {
        ...
        {
            string access_token = Common.Request.QueryString["token"].String;
            string openid = Common.Request.Form["openid"].String;
            EntitiesInfo.Organization org = getOrgan();
            //获取qq登录账户的信息
            EntitiesInfo.Accounts acc = Business.Do<IAccounts>().Account4QQ(openid);
            if (acc == null)
            {
                ...
                Common.Request.LoadFile(acc.Ac_Photo, photoPath);
