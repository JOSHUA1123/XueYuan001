    ...
    public class RechargeCom : IRecharge
    {
        ...
        public void RechargeSetSave(RechargeSet entity)
        {
            using (DbTrans tran = Gateway.Default.BeginTrans())
            {
                try
                {
                    RechargeSet rs = tran.From<RechargeSet>().Where(RechargeSet._.Rs_ID == entity.Rs_ID).ToFirst<RechargeSet>();
                    //如果新增了充值码
                    if (rs.Rs_Count < entity.Rs_Count) _RechargeCodeBuilds(entity, entity.Rs_Count - rs.Rs_Count);
                    //如果减少的充值码
                    if (rs.Rs_Count > entity.Rs_Count)
                    {
                        RechargeCode[] rcs = tran.From<RechargeCode>().Where(RechargeCode._.Rs_ID == entity.Rs_ID && RechargeCode._.Rc_IsUsed == false).ToArray<RechargeCode>(rs.Rs_Count - entity.Rs_Count);
        ...
        {
            //是否被禁用
            RechargeSet set = Gateway.Default.From<RechargeSet>().Where(RechargeSet._.Rs_ID == entity.Rs_ID).ToFirst<RechargeSet>();
        ...
        public string RechargeCode4Excel(string path, int orgid, int rsid)
        {
            ...
            string confing = Common.App.Get["ExcelInputConfig"].VirtualPath + "充值码.xml";
            xmldoc.Load(Common.Server.MapPath(confing));
            XmlNodeList nodes = xmldoc.GetElementsByTagName("item");
            //创建工作簿对象
            RechargeSet rs = Gateway.Default.From<RechargeSet>().Where(RechargeSet._.Rs_ID == rsid).ToFirst<RechargeSet>();
