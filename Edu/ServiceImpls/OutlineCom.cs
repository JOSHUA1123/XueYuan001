...
namespace ServiceImpls
{
    public class OutlineCom : IOutline
    {
        ...
        public void OutlineAdd(Outline entity)
        {
            using (DbTrans tran = Gateway.Default.BeginTrans())
            {
                try
                {
                    ...
                    {
                        Course cou = Business.Do<ICourse>().CourseSingle(entity.Cou_ID);
                    ...
                    if (string.IsNullOrWhiteSpace(entity.Ol_UID))
                        entity.Ol_UID = Common.Request.UniqueID();
                    ////层级
        ...
        public Outline OutlineBatchAdd(int orgid, int sbjid, int couid, string names)
        {
            ...
            {
                Outline current = OutlineIsExist(orgid, sbjid, couid, pid, listName[i]);
                if (current == null)
                {
                    current = new Outline();
        ...
        public string OutlineExport4Excel(string path, int couid)
        {
            ...
            string confing = Common.App.Get["ExcelInputConfig"].VirtualPath + "课程章节.xml";
            xmldoc.Load(Common.Server.MapPath(confing));
            XmlNodeList nodes = xmldoc.GetElementsByTagName("item");
            //当前课程的所有章节
            Outline[] outls = this.OutlineCount(couid, -1, null, -1);
            ...
            Course course = Business.Do<ICourse>().CourseSingle(couid);
        ...
        public void OutlineDelete(Outline entity)
        {
            ...
            List<Accessory> acs = Business.Do<IAccessory>().GetAll(entity.Ol_UID);
            using (DbTrans tran = Gateway.Default.BeginTrans())
            {
                try
                {
        ...
        public Outline OutlineSingle(int identify)
        {
            ...
            //{
            //    //从缓存中读取
            //    List<Outline> list = Common.Cache<Outline>.Data.List;
        ...
        public string OutlineName(int identify)
        {
            ...
            Outline tm = Gateway.Default.From<Outline>().Where(Outline._.Ol_ID == entity.Ol_PID).ToFirst<Outline>();
            while (tm != null)
            {
        ...
        /// <returns></returns>
        public DataTable OutlineTree(Outline[] outlines)
        {
            //计算树形的运算时间
            //DateTime beforDT = System.DateTime.Now;
            //Common.Log.Debug(this.GetType().Name, "---开始计算章节树形：" + beforDT.ToString("yyyy年MM月dd日 hh:mm:ss"));
            ...
            //{
            //    //Common.Log.Debug(this.GetType().Name, string.Format("计算章节树形,耗时：{0}ms", ts.TotalMilliseconds));
            //}
        ...
        public int OutlineCleanup(int couid)
        {
            WhereClip wc = new WhereClip();
            if (couid > 0) wc &= Outline._.Cou_ID == couid;
            List<Outline> outs = Gateway.Default.From<Outline>().Where(wc && Outline._.Ol_PID != 0).ToList<Outline>();
            int count = 0;
        ...
        //        try
        //        {
        //            Common.Cache<Outline>.Data.Clear();
        //        }
        //        catch
        //        {
        //        }
        //        finally
        //        {
        //            Outline[] outls = Gateway.Default.From<Outline>().ToArray<Outline>();
        //            Common.Cache<Outline>.Data.Fill(outls);
        //        }
        //        ////计算每个章节下的试题数
        ...
        //        //}                
        //        return Common.Cache<Outline>.Data.List;
        //    }
        //}
        /// <summary>
        /// 获取指定个数的课程列表
        ...
        public bool OutlineUp(int couid, int id)
        {
            ...
            else
            {
                ...
                //没有当前等级哥哥对像
                Outline top = Gateway.Default.From<Outline>().Where(Outline._.Ol_ID == current.Ol_PID).ToFirst<Outline>();
                if (top == null) return false;
                //父级的哥哥
                Outline topPrev = Gateway.Default.From<Outline>()
        ...
        public bool OutlineDown(int couid, int id)
        {
            ...
            else
            {
                ...
                //父级的弟弟
                Outline topNext = Gateway.Default.From<Outline>()
                ...
                //父级弟弟的子级
                Outline[] child = this.OutlineCount(couid, topNext.Ol_ID, null, -1);
                using (DbTrans tran = Gateway.Default.BeginTrans())
                {
                    try
                    {
        ...
        public bool OutlineToLeft(int couid, int id)
        {
            ...
            Outline parent = Gateway.Default.From<Outline>().Where(Outline._.Ol_ID == current.Ol_PID).ToFirst<Outline>();
            //顶级列表
            Outline[] top = Gateway.Default.From<Outline>()
                .Where(Outline._.Ol_Tax > parent.Ol_Tax && Outline._.Cou_ID == current.Cou_ID && Outline._.Ol_PID == parent.Ol_PID)
                .OrderBy(Outline._.Ol_Tax.Asc).ToArray<Outline>();
            //当前父的同级中，比自己大的子级（即自身后面的）
            Outline[] child = Gateway.Default.From<Outline>()
        ...
        public bool OutlineToRight(int couid, int id)
        {
            //当前对象
            Outline current = Gateway.Default.From<Outline>().Where(Outline._.Ol_ID == id).ToFirst<Outline>();
            if (current.Ol_Tax == 1) throw new Common.ExceptionForAlert("最顶级的第一个章节，不可以降级");
            //上一个对象，即兄长对象；兄长不存则直接返回false;
            Outline prev = Gateway.Default.From<Outline>()
                .Where(Outline._.Ol_Tax < current.Ol_Tax && Outline._.Cou_ID == current.Cou_ID && Outline._.Ol_PID == current.Ol_PID)
                .OrderBy(Outline._.Ol_Tax.Desc).ToFirst<Outline>();
        ...
        /// <param name="entity"></param>
        /// <returns></returns>
        private int _ClacLevel(Outline entity)
        {
            ...
            Gateway.Default.Save<Outline>(entity);
            Outline[] childs = Gateway.Default.From<Outline>().Where(Outline._.Ol_PID == entity.Ol_ID).ToArray<Outline>();
        ...
        /// <param name="entity"></param>
        /// <returns></returns>
        private string _ClacXPath(Outline entity)
        {
        ...
        public void OnSave(object sender, EventArgs e)
        {
            if (sender != null)
            {
                ...
                //Outline old = this.OutlineSingle(ol.Ol_ID);
                //ol.Ol_QuesCount = Business.Do<IOutline>().QuesOfCount(ol.Ol_ID, -1, true, true);
                //Common.Cache<Outline>.Data.Update(old, ol);
            }
            else
            {
                //填充章节数据到缓存
        ...
        public void OnAdd(object sender, EventArgs e)
        {
            ...
            if (sender != null)
            {
                //if (!(sender is Outline)) return;
                //Outline ol = (Outline)sender;
                //Common.Cache<Outline>.Data.Add(ol);
            }
            if (Add != null)
                Add(sender, e);
        }
        public void OnDelete(object sender, EventArgs e)
        {
            ...
            if (sender != null)
            {
                //if (!(sender is Outline)) return;
                //Outline ol = (Outline)sender;
                //Common.Cache<Outline>.Data.Delete(ol);
                ////重建试题缓存（取所有试题进缓存）
                //new Thread(new ThreadStart(() =>
                //{
                //    Questions[] ques = Business.Do<IQuestions>().QuesCount(-1, null, -1);
                //    ServiceImpls.QuestionsMethod.QuestionsCache.Singleton.Delete("all");
                //    ServiceImpls.QuestionsMethod.QuestionsCache.Singleton.Add(ques, int.MaxValue, "all");
                //})).Start();
            }
            if (Delete != null)
                Delete(sender, e);
