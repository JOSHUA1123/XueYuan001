...
namespace ServiceImpls
{
    public class ExaminationCom : IExamination
    {
            ...
            using (DbTrans tran = Gateway.Default.BeginTrans())
            {
                ...
                {                   
                    if (items != null)
                    {
                        ...
                        foreach (Examination it in items)
                        {
        ...
        {
            Examination exam = this.ExamSingle(identify);
            using (DbTrans tran = Gateway.Default.BeginTrans())
            {
                try
                {
        ...
        {
            Examination exam = this.ExamSingle(id);
        ...
        {
            //所在班组的考试
            StudentSort[] sts = Gateway.Default.From<StudentSort>().InnerJoin<ExamGroup>("[ExamGroup].[Sts_ID] = [StudentSort].[Sts_ID]")
        ...
        public List<Examination> ExamCount(int orgid, bool? isUse, int count)
        {
            ...
            Examination[] all = Gateway.Default.From<Examination>().Where(wc).OrderBy(Examination._.Exam_Date.Desc).ToArray<Examination>(count);
            List<Examination> exams = new List<Examination>();
            foreach (Examination t in all)
                _GetSelfExam_Add(exams, t);
            return exams;
        }
        public List<Examination> GetSelfExam(int stid, DateTime? start, DateTime? end)
        {
            Accounts acc = Gateway.Default.From<Accounts>().Where(Accounts._.Ac_ID == stid).ToFirst<Accounts>();
            ...
            #endregion
            //全员参加的考试
            Examination[] all = Gateway.Default.From<Examination>().Where(wc && Examination._.Exam_GroupType == 1).ToArray<Examination>();
            //所在学生组的考试            
            Examination[] group = Gateway.Default.From<Examination>().InnerJoin<ExamGroup>("[ExamGroup].[Exam_UID] = [Examination].[Exam_UID]")
                .Where(wc && Examination._.Exam_GroupType == 2 && ExamGroup._.Sts_ID == acc.Sts_ID).ToArray<Examination>();          
            //合并到一起
            List<Examination> exams = new List<Examination>();
            foreach (Examination t in all)
                _GetSelfExam_Add(exams, t);
            foreach (Examination t in group)
                _GetSelfExam_Add(exams, t);
            //排序
        ...
        public List<Examination> GetCountExam(int stid, DateTime? start, DateTime? end, bool? isUse, int count)
        {
            ...
            count = count > 0 ? count : int.MaxValue;
            //全员参加的考试
            Examination[] all = Gateway.Default.From<Examination>().Where(wc).ToArray<Examination>();
            List<Examination> exams = new List<Examination>();
            foreach (Examination t in all)
                _GetSelfExam_Add(exams, t);
            //排序
        ...
        public ExamResults ResultAdd(ExamResults result)
        {
            ...
            ExamResults exr = Gateway.Default.From<ExamResults>().Where(wc).ToFirst<ExamResults>();
            if (exr != null)
            {
        ...
        {
            ExamResults exr = this.ResultSingle(result.Exam_ID, result.Tp_Id, result.Ac_ID);
            if (exr == null)
            {
                exr = result;
                exr.Exr_CrtTime = DateTime.Now;
                exr.Exr_SubmitTime = DateTime.Now;
                Accounts st= Gateway.Default.From<Accounts>().Where(Accounts._.Ac_ID==result.Ac_ID).ToFirst<Accounts>();
                Organization org = Gateway.Default.From<Organization>().Where(Organization._.Org_ID == st.Org_ID).ToFirst<Organization>();
                if (org != null)
                {
        ...
        public ExamResultsTemp ExamResultsTempSingle(int examid, int tpid, int accid)
        {
            ...
            ExamResultsTemp exr = Gateway.Default.From<ExamResultsTemp>().Where(wc).ToFirst<ExamResultsTemp>();
            return exr;
        }
        /// <summary>
        ...
        public ExamResults ResultSingle(int examid, int tpid, int acid)
        {
            ...
            ExamResults exr = Gateway.Default.From<ExamResults>().Where(wc).OrderBy(ExamResults._.Exr_CrtTime.Desc).ToFirst<ExamResults>();            
            return exr;
        }
        /// <summary>
        /// 从缓存中获取考试答题信息
        ...
        public List<Questions> QuesForResults(string results)
        {
            ...
            for (int i = 0; i < s.Length; i++)
            {
                ...
                Questions q = Gateway.Default.From<Questions>().Where(Questions._.Qus_ID == id).ToFirst<Questions>();
                if (q == null) continue;
                quesList.Add(q);
            }
            //设置该题分数
            foreach (Questions q in quesList)
            {
                for (int i = 0; i < nodeList.Count; i++)
                {
        ...
        /// <param name="result"></param>
        /// <returns></returns>
        public ExamResults ClacScore(ExamResults result)
        {
        ...
        public static float _ClacScore(string resultXML,out string outXml)
        {
            ...
            for (int i = 0; i < quesNodes.Count; i++)
            {
                ...
                for (int j = 0; j < qnode.Count; j++)
                {
                    ...
                    if (!isSucess)
                    {
                        Student_Ques sq = new Student_Ques();
                        //学生Id
                        sq.Ac_ID = info.stid;
                        //试题id与类型
                        sq.Qus_ID = ques.id;
        ...
        //    }
        //    Depart[] deps = this.GroupForDepart(theme.Exam_UID);
        ...
        //            results[i] = _ClacScore(results[i]);
        //    }
        //    Team[] teams = this.GroupForTeam(theme.Exam_UID);
