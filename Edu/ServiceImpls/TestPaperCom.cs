...
namespace ServiceImpls
{
    public class TestPaperCom : ITestPaper
    {
        ...
        public void PagerDelete(int identify)
        {
            using (DbTrans tran = Gateway.Default.BeginTrans())
            {
                try
                {
                    ...
                    if (exam != null) throw new Common.ExceptionForPrompt("该试卷已被考试采用，不能删除");
        ...
        {
            //获取试题项,例如单选题、多选题
            TestPaperItem[] tpi = this.GetItemForAll(tp);           
            List<TestPaperItem> list = new List<TestPaperItem>();
            foreach (TestPaperItem t in tpi)
            {
                if (t.TPI_Count > 0) list.Add(t);
            ...
            foreach (TestPaperItem t in tpi)
            {
                ...
                if (count < 1) continue;
                //当前类型的试题
                Questions[] ques = Business.Do<IQuestions>().QuesRandom(tp.Org_ID, (int)tp.Sbj_ID, tp.Cou_ID, -1, type, -1, -1, true, count);
        ...
        {
            //获取试题项,例如单选题、多选题
            TestPaperItem[] tpi = this.GetItemForOlPercent(tp);
            List<TestPaperItem> list = new List<TestPaperItem>();
            foreach (TestPaperItem t in tpi) if (t.TPI_Percent > 0) list.Add(t);
            tpi = _getItemCoutomOrder(list.ToArray());
            //***************  开始抽取试题
        ...
        /// <returns></returns>
        private Questions[] clacScore(Questions[] ques, float total)
        {
            float surplus = total;
        ...
        {
            TestPaper mm = Gateway.Default.From<TestPaper>().Where(TestPaper._.Tp_Id == identify).ToFirst<TestPaper>();
            //参考人次
        ...
        /// <param name="tr_CrtTime"></param>
        /// <returns></returns>
        public TestResults TestResultsInfo(int ac_ID, DateTime? tr_CrtTime)
        {
            return Gateway.Default.From<TestResults>().Where(TestResults._.Ac_ID == ac_ID&& TestResults._.Tr_CrtTime==tr_CrtTime).ToFirst<TestResults>();
        }
