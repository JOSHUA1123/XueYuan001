...
namespace ServiceImpls
{
    public class StudentCom : IStudent
    {
        ...
        public int SortDelete(int identify)
        {
            ...
            if (st != null) throw new Common.ExceptionForAlert("当前学员分类下有学员信息");
        ...
        public int StudentAdd(Accounts entity)
        {
            ...
            if (string.IsNullOrWhiteSpace(entity.Ac_AccName))
            {
                ...
                {
                    entity.Ac_AccName = Common.Request.UniqueID();
                }
            }
            //如果密码为空
        ...
        private string getSha1(Accounts entity)
        {
            ...
            for (int i = 0; i < arr.Length; i++)
                sha1 += arr[i];
            sha1 = new Common.Param.Method.ConvertToAnyValue(sha1).SHA1;
        ...
        {
            Accounts st = this.StudentSingle(identify);
            if (st == null) return;
            this.StudentDelete(st);
        }        
        public void StudentDelete(string accname, int orgid)
        {
            Accounts st = this.StudentSingle(accname, orgid);
            if (st == null) return;
            this.StudentDelete(st);
        }

        public void StudentDelete(Accounts entity)
        {
            using (DbTrans tran = Gateway.Default.BeginTrans())
            {
        ...
        public void StudentDelete(Accounts entity, DbTrans tran)
        {
        ...
        {
            Accounts st= Gateway.Default.From<Accounts>().Where(Accounts._.Ac_ID == identify).ToFirst<Accounts>();
            if (st != null)
            {
        ...
        #region 学员登录与在线记录
        private LogForStudentOnlineService logForStudentOnlineService = new LogForStudentOnlineService();
        /// <summary>
        /// 添加登录记录
        /// </summary>
        /// <param name="entity"></param>
        ...
        {
            LogForStudentOnline entity = new LogForStudentOnline();
            
            //Entities.LogForStudentOnline entity = new Entities.LogForStudentOnline();
            //当前机构
            Organization org = Business.Do<IOrganization>().OrganCurrent();
            ...
            //登录信息
            entity.Lso_IP = Common.Browser.IP;
            entity.Lso_OS = Common.Browser.OS;
            entity.Lso_Browser = Common.Browser.Name + " " + Common.Browser.Version;
            entity.Lso_Platform = Common.Browser.IsMobile ? "Mobi" : "PC";

            //logForStudentOnlineService.Add(entity);
        ...
        public void LogForLoginFresh(int interval, string plat)
        {
            ...
            //学员信息
            Accounts st = Extend.LoginState.Accounts.CurrentUser;
            if (st == null) return;
            //取当前登录记录
            int acid = st.Ac_ID;
            string uid = Extend.LoginState.Accounts.UID;
            LogForStudentOnline entity = this.LogForLoginSingle(acid, uid, plat);
            ...
            else
            {
                ...
                //Entities.LogForStudentOnline logEntity = new Entities.LogForStudentOnline();
                //logEntity.Org_ID = entity.Org_ID;
                //logEntity.Ac_ID = entity.Ac_ID;
                //logEntity.Ac_AccName = entity.Ac_AccName;
                //logEntity.Ac_Name = entity.Ac_Name;
                //logEntity.Lso_UID = entity.Lso_UID;
                //logEntity.Lso_LoginDate = entity.Lso_LoginDate;
                //logEntity.Lso_LoginTime = entity.Lso_LoginTime;
                //logEntity.Lso_LastTime = entity.Lso_LastTime;
                //logEntity.Lso_LogoutTime = entity.Lso_LogoutTime;
                //logEntity.Lso_CrtTime = entity.Lso_CrtTime;
                //logEntity.Lso_OnlineTime = entity.Lso_OnlineTime;
                //logEntity.Lso_BrowseTime = entity.Lso_BrowseTime;
                //logEntity.Lso_IP = entity.Lso_IP;
                //logEntity.Lso_Browser = entity.Lso_Browser;
                //logEntity.Lso_Platform = entity.Lso_Platform;
                //logEntity.Lso_OS = entity.Lso_OS;
                //logForStudentOnlineService.Add(logEntity);
        ...
        public void LogForStudyFresh(int couid, int olid, Accounts st, int playTime, int studyInterval, int totalTime)
        {
            if (st == null) return;
            //当前章节的学习记录
            LogForStudentStudy entity = this.LogForStudySingle(st.Ac_ID, olid);
            if (entity == null)
            {
        ...
        private LogForStudentStudyService logForStudentStudyService=new LogForStudentStudyService();
        /// <summary>
        /// 记录学员学习时间
        /// </summary>
        /// <param name="couid"></param>
        ...
        public double LogForStudyUpdate(int couid, int olid, Accounts st, int playTime, int studyTime, int totalTime)
        {
            ...
            LogForStudentStudy entity = Gateway.Default.FromSql(sql).ToFirst<LogForStudentStudy>();
            if (entity == null)
            {
                //当前机构
                Organization org = Business.Do<IOrganization>().OrganCurrent();
                entity = new LogForStudentStudy();
                entity.Lss_UID = Common.Request.UniqueID();
            ...
            //登录信息
            entity.Lss_IP = Common.Browser.IP;
            entity.Lss_OS = Common.Browser.OS;
            entity.Lss_Browser = Common.Browser.Name + " " + Common.Browser.Version;
            entity.Lss_Platform = Common.Browser.IsMobile ? "Mobi" : "PC";

            Entities.LogForStudentStudy logForStudentStudy = new Entities.LogForStudentStudy();
            logForStudentStudy.Org_ID = entity.Org_ID;
            logForStudentStudy.Ac_ID = entity.Ac_ID;
            logForStudentStudy.Ac_AccName = entity.Ac_AccName;
            logForStudentStudy.Ac_Name = entity.Ac_Name;
            logForStudentStudy.Cou_ID = entity.Cou_ID;
            logForStudentStudy.Ol_ID = entity.Ol_ID;
            logForStudentStudy.Lss_UID = entity.Lss_UID;
            logForStudentStudy.Lss_CrtTime = entity.Lss_CrtTime;
            logForStudentStudy.Lss_PlayTime = entity.Lss_PlayTime;
            logForStudentStudy.Lss_StudyTime = entity.Lss_StudyTime;
            logForStudentStudy.Lss_Duration = entity.Lss_Duration;
            logForStudentStudy.Lss_LastTime = entity.Lss_LastTime;
            logForStudentStudy.Lss_IP = entity.Lss_IP;
            logForStudentStudy.Lss_Browser = entity.Lss_Browser;
            logForStudentStudy.Lss_Platform = entity.Lss_Platform;
            logForStudentStudy.Lss_OS = entity.Lss_OS;

            //保存到数据库
            logForStudentStudyService.Add(logForStudentStudy);
	...
	(select cou_id, max(lastTime) as 'lastTime',sum(studyTime) as 'studyTime',		
        ...
        public Questions[] QuesAll(int acid, int sbjid, int couid, int type)
        {
            ...
            return Gateway.Default.From<Questions>()
                .InnerJoin<Student_Ques>("[Questions].[Qus_ID] = [Student_Ques].[Qus_ID]")
...
on c.qus_id=sq.qus_id order by sq.count desc";
        ...
        public Questions[] CollectAll4Ques(int acid, int sbjid, int couid, int type)
        {
            ...
            return Gateway.Default.From<Questions>()
                .InnerJoin<Student_Collect>("[Questions].[Qus_ID] = [Student_Collect].[Qus_ID]")
        ...
        public Questions[] NotesCount(int stid, int couid, int type, int count)
        {
            ...
            if (type > 0) wc.And(Student_Notes._.Qus_Type == type);
            return Gateway.Default.From<Questions>()
                .InnerJoin<Student_Notes>("[Questions].[Qus_ID] = [Student_Notes].[Qus_ID]")
