...
namespace ServiceImpls
{
    public class UserCom : IUser
    {
        ...
        {
            UserGroup current = this.GetGroupSingle(id);
            //当前对象排序号
            int orderValue = (int)current.UGrp_Tax; 
            //上一个对象，即兄长对象；存在当前优先级
            UserGroup up = Gateway.Default.From<UserGroup>().Where(UserGroup._.UGrp_Tax > orderValue).OrderBy(UserGroup._.UGrp_Tax.Asc).ToFirst<UserGroup>();
            if (up == null)
            {
        ...
        public bool GroupRemoveDown(int id)
        {
            ...
            int orderValue = (int)current.UGrp_Tax;
            //下一个对象，即弟弟对象；
            UserGroup down = Gateway.Default.From<UserGroup>().Where(UserGroup._.UGrp_Tax < orderValue).OrderBy(UserGroup._.UGrp_Tax.Desc).ToFirst<UserGroup>();
            if (down == null)
            {
        ...
        public User[] GetUserPager(int grpid, int size, int index, out int countSum)
        {
            ...
            User[] ac = Gateway.Default.From<User>().Where(wc).OrderBy(User._.User_RegTime.Desc).ToArray<User>(size, (index - 1) * size);
            return ac;
        }
