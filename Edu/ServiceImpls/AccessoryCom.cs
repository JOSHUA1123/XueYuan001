...
namespace ServiceImpls
{
    public class AccessoryCom : IAccessory
    {
        ...
        public void Add(Accessory entity)
        {
           ...
           Outline outline = Gateway.Default.From<Outline>().Where(Outline._.Ol_UID == entity.As_Uid).ToFirst<Outline>();
        ...
        public void Delete(int identify)
        {
            ...
            //如果是视频,设置该视频所在的章节是否有视频
           Outline outline = Gateway.Default.From<Outline>().Where(Outline._.Ol_UID == ac.As_Uid).ToFirst<Outline>();
            if (outline != null) Business.Do<IOutline>().OutlineSave(outline);
        }
        /// <summary>
        /// 删除，按系统唯一id
        ...
        {           
            ...
            //如果是视频,设置该视频所在的章节是否有视频
           Outline outline = Gateway.Default.From<Outline>().Where(Outline._.Ol_UID == uid).ToFirst<Outline>();
            if (outline != null) Business.Do<IOutline>().OutlineSave(outline);
        }
        public void Delete(string uid, DataBaseInfo.DbTrans tran)
        {
            List<Accessory> acs = this.GetAll(uid);
            if (acs == null) return;
            foreach (Accessory ac in acs)
