...
namespace ServiceImpls
{
    public partial class ContentsCom : IContents
    {
        ...
        {
            Product entity = this.ProductSingle(identify);
            using (DbTrans tran = Gateway.Default.BeginTrans())
            {
                try
                {
        ...
        public void ProductDeleteAll(int orgid, int colid)
        {
            ...
            Product[] entities = Gateway.Default.From<Product>().Where(wc).ToArray<Product>();
        ...
        private void _ProductDelete(Product entity, DbTrans tran)
        {
            ...
            // 删除产品图片
            //ProductPicture[] pps = this.PictureAll(entity.Pd_Id, null);
            //foreach (ProductPicture p in pps)
            //{
            //    this._PictureDelete(p);
            //}
            //删除产品主信息
            string resPath = Common.Upload.Get["Product"].Physics;
