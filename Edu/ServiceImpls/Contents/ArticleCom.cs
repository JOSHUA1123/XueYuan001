...
namespace ServiceImpls
{
    public partial class ContentsCom : IContents
    {
        ...
        public int ArticleAdd(Article entity)
        {
            ...
            //所属栏目
           Columns nc = Business.Do<IColumns>().Single((int)entity.Col_Id);
            ...
            if (entity.Art_IsTop)
            {
                ...
                if (count >= topNumber)
                {
                    Article[] tna = Gateway.Default.From<Article>().Where(Article._.Art_IsTop == true).OrderBy(Article._.Art_CrtTime.Asc).ToArray<Article>(count - topNumber + 1);
                    if (tna != null)
                    {
            ...
            if (entity.Art_IsRec)
            {
                ...
                {
                    Article[] tna = Gateway.Default.From<Article>().Where(Article._.Art_IsRec == true).OrderBy(Article._.Art_CrtTime.Asc).ToArray<Article>(reccount - recNumber + 1);
                    if (tna != null)
                    {
                        foreach (Article na in tna)
                        {
        ...
        public void ArticleDelete(Article entity, DbTrans tran)
        {
            ...
            try
            {
                ...
                //删除图片文件
                string img = Common.Upload.Get[_artUppath].Physics + entity.Art_Logo;
        ...
        public void ArticleDeleteAll(int orgid, int colid)
        {
            ...
            Article[] entities = Gateway.Default.From<Article>().Where(wc).ToArray<Article>();
        ...
        public Article ArticlePrev(int identify, int orgid)
        {
            ...
            Article art = this.ArticleSingle(identify);
        ...
        {
            return Gateway.Default.From<Special>().InnerJoin<Special_Article>("[Special_Article].[Sp_Id] = [Special].[Sp_Id]")
        ...
        public Article[] ArticleCount(int orgid, int colid, int topNum, string order)
        {
           ...
           Article[] arts = Gateway.Default.From<Article>().Where(wc).OrderBy(wcOrder && Article._.Art_PushTime.Desc && Article._.Art_CrtTime.Desc).ToArray<Article>(topNum);
            return arts;
        }
