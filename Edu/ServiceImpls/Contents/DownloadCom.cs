...
namespace ServiceImpls
{
    public partial class ContentsCom : IContents
    {
        ...
        {
            Download entity = this.DownloadSingle(identify);
        ...
        public void DownloadDeleteAll(int orgid, int colid)
        {
            ...
            Download[] entities = Gateway.Default.From<Download>().Where(wc).ToArray<Download>();
            foreach (Download entity in entities)
            {
                DownloadDelete(entity);
            }
        }
