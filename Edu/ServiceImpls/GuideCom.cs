...
namespace ServiceImpls
{
    public class GuideCom :IGuide
    {
        ...
        public void GuideAdd(Guide entity)
        {
            ...
            //所属栏目
            GuideColumns nc = this.ColumnsSingle((int)entity.Gc_ID);
            ...
            using (DbTrans tran = Gateway.Default.BeginTrans())
            {
                try
                {
                    if (!string.IsNullOrWhiteSpace(entity.Gu_Uid))
                    {
                        ...
                        //删除图片文件
                        string img = Common.Upload.Get[_artUppath].Physics + entity.Gu_Logo;
        ...
        {
            Guide guide = this.GuideSingle(identify);
            GuideDelete(guide);
        }
        /// <summary>
        /// 当前新闻的上一条新闻
        ...
        {
            GuideColumns old = this.ColumnsSingle(entity.Gc_ID);
        ...
        {
            GuideColumns[] cols = GetColumnsChild(entity.Cou_ID, entity.Gc_ID, null);
            foreach (GuideColumns cl in cols)
                ColumnsDelete(cl);
        ...
        {
            GuideColumns col = ColumnsSingle(identify);
            ColumnsDelete(col);
        }        
        /// <summary>
        /// 获取单一实体对象，按主键ID；
