    ...
    public class EmployeeCom : IEmployee
    {
        ...
        public int Add(EmpAccount entity)
        {
            ...
            if (string.IsNullOrWhiteSpace(entity.Acc_Pw))
                entity.Acc_Pw = Common.Login.Get["Admin"].DefaultPw.MD5;
            else
                entity.Acc_Pw = new Common.Param.Method.ConvertToAnyValue(entity.Acc_Pw).MD5;
            //员所在院系、所处岗位、所在机构
        ...
        public EmpAccount GetSingle(int orgid, string acc, string pw)
        {
            ...
            wc.And(EmpAccount._.Acc_Pw == new Common.Param.Method.ConvertToAnyValue(pw).MD5);
            //先用帐号
        ...
        public EmpAccount EmpLogin(string acc, string pw, int orgid)
        {
            ...
            string pwMd5= new Common.Param.Method.ConvertToAnyValue(pw).MD5;
            wc.And(EmpAccount._.Acc_Pw == pwMd5);
            EmpAccount entity = null;
            if (entity == null) entity = Gateway.Default.From<EmpAccount>().Where(EmpAccount._.Acc_AccName == acc).ToFirst<EmpAccount>();
            if (entity == null) entity = Gateway.Default.From<EmpAccount>().Where( EmpAccount._.Acc_MobileTel == acc).ToFirst<EmpAccount>();
            if (entity == null) entity = Gateway.Default.From<EmpAccount>().Where(EmpAccount._.Acc_IDCardNumber == acc).ToFirst<EmpAccount>();
            return entity;
        }
        /// <summary>
        /// 获取当前员工所在的院系
        ...
        public bool IsForRoot(int identify)
        {
            ...
            //先获取员工对象，如果为空则返回false;
            EmpAccount ea = this.GetSingle(identify);
            Organization org = Gateway.Default.From<Organization>().Where(Organization._.Org_IsRoot == true && Organization._.Org_ID == ea.Org_ID).ToFirst<Organization>();
            return org != null;
        }
        /// <summary>
        /// 当前用户是否为超级管理员
