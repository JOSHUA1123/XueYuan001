...
namespace ServiceImpls
{
    public class MessageBoardCom : IMessageBoard
    {
        ...
        public void ThemeAdd(MessageBoard entity)
        {
            ...
            entity.Mb_IP = Common.Request.IP.IPAddress;
            //uid
            if (string.IsNullOrWhiteSpace(entity.Mb_UID))
            {
                entity.Mb_UID = Common.Request.UniqueID();
            }
        ...
        {
            MessageBoard tm = this.ThemeSingle(identify);
        ...
        public void AnswerAdd(MessageBoard entity)
        {
            ...
            MessageBoard theme = Gateway.Default.From<MessageBoard>().Where(MessageBoard._.Mb_IsTheme == true && MessageBoard._.Mb_UID == entity.Mb_UID).ToFirst<MessageBoard>();
        ...
        public void AnswerDelete(int identify)
        {
            ...
            {
                MessageBoard theme = Gateway.Default.From<MessageBoard>().Where(MessageBoard._.Mb_IsTheme == true && MessageBoard._.Mb_UID == mb.Mb_UID).ToFirst<MessageBoard>();
                using (DbTrans tran = Gateway.Default.BeginTrans())
                    try
                    {
                        theme.Mb_AnsTime = DateTime.Now;
        ...
        public MessageBoard[] ListPager(string uid, int size, int index, out int countSum)
        {
            ...
            countSum = Gateway.Default.Count<MessageBoard>(wc);
            MessageBoard[] msg = Gateway.Default.From<MessageBoard>().Where(wc).OrderBy(MessageBoard._.Mb_CrtTime.Asc).ToArray<MessageBoard>(size, (index - 1) * size);
            //把主题放到最前面
            for (int i = 0; i < msg.Length; i++)
            {
                if (msg[i].Mb_IsTheme == true)
                {
                    MessageBoard tm = msg[0];
                    msg[0] = msg[i];
                    msg[i] = tm;
                    break;
                }
