...
namespace ServiceImpls.QuestionsMethod
{
    /// <summary>
    /// 试题的缓存
    /// </summary>
    public class QuestionsCache
    {
        ...
        /// <returns></returns>
        public Questions GetSingle(int qid)
        {
            Questions ques = null;
            for (int i = 0; i < list.Count; i++)
            {
        ...
        /// <param name="uid"></param>
        /// <returns></returns>
        public Questions GetSingle(string uid)
        {
            Questions ques = null;
            for (int i = 0; i < list.Count; i++)
            {
        ...
        /// <param name="qid"></param>
        /// <returns></returns>
        public void UpdateSingle(Questions ques)
        {            
            for (int i = 0; i < list.Count; i++)
            {
                for (int j = 0; j < list[i].Questions.Count; j++)
        ...
        public List<Questions> GetQuestions(string uid)
        {
            ...
            if (uid == "all")
            {
                if (qci == null)
                {
                    Questions[] ques = Business.Do<IQuestions>().QuesCount(-1, true, -1);
                    this.Delete("all");
    ...
    public class QuestionsCache_Item{
        ...
        private ExamResults _result=null;
        /// <summary>
        /// 考试的答题信息
        /// </summary>
        public ExamResults Result
        {
            get { return _result; }
            set { _result = value; }
        }
