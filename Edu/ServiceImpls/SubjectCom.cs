...
namespace ServiceImpls
{
    public class SubjectCom : ISubject
    {
        ...
        public Subject SubjectBatchAdd(int orgid, string names)
        {
            ...
            {
                Subject current = SubjectIsExist(orgid, pid, listName[i]);
                if (current == null)
                {
                    current = new Subject();
        ...
        public void SubjectSave(Subject entity)
        {
            ...
            if (entity.Dep_Id > 0)
            {
                Depart depart = Gateway.Default.From<Depart>().Where(Depart._.Dep_Id == entity.Dep_Id).ToFirst<Depart>();
                if (depart != null)
                {
        ...
        public void SubjectDelete(int identify)
        {
            ...
            if (count > 0) throw new Common.ExceptionForPrompt("当前专业下包括" + count + "道试题，请清空后再删除！");
            //是否存在课程
            count = Gateway.Default.Count<Course>(Course._.Sbj_ID == identify);
            if (count > 0) throw new Common.ExceptionForPrompt("当前专业下包括" + count + "个课程，请清空后再删除！");
            //是否有下级
            count = Gateway.Default.Count<Subject>(Subject._.Sbj_PID == identify);
            if (count > 0) throw new Common.ExceptionForPrompt("当前专业下包括" + count + "个子专业，请清空后再删除！");
            if (count < 1)
            {
        ...
        {
            //清理课程
            Course[] cous = Gateway.Default.From<Course>().Where(Course._.Sbj_ID == identify).ToArray<Course>();
            if (cous != null && cous.Length > 0)
            {
                foreach (Course c in cous)
                    Business.Do<ICourse>().CourseClear(c.Cou_ID);
            }
            //清理试题
            Questions[] ques = Gateway.Default.From<Questions>().Where(Questions._.Sbj_ID == identify).ToArray<Questions>();
        ...
        public string SubjectName(int identify)
        {
            ...
            Subject tm = Gateway.Default.From<Subject>().Where(Subject._.Sbj_ID == entity.Sbj_PID).ToFirst<Subject>();
            while (tm != null)
            {
        ...
        /// <returns></returns>
        private int _ClacLevel(Subject entity)
        {
            ...
            Subject[] childs = Gateway.Default.From<Subject>().Where(Subject._.Sbj_PID == entity.Sbj_ID).ToArray<Subject>();
        ...
        /// <param name="entity"></param>
        /// <returns></returns>
        private string _ClacXPath(Subject entity)
        {
