...
namespace ServiceImpls
{
    public class TaskCom : ITask
    {
        ...
        /// <returns></returns>
        private Task setEmployeeValue(Task entity)
        {
            //设置承接任务的员工名称
            EmpAccount ea = Gateway.Default.From<EmpAccount>().Where(EmpAccount._.Acc_Id == entity.Task_WorkerId).ToFirst<EmpAccount>();
        ...
        {
            Task current = this.GetSingle(id);
            //当前对象排序号
            int orderValue = (int)current.Task_Tax; 
            //上一个对象，即兄长对象；存在当前优先级
            Task up = Gateway.Default.From<Task>().Where(Task._.Task_Tax > orderValue && Task._.Task_Level == current.Task_Level).OrderBy(Task._.Task_Tax.Asc).ToFirst<Task>();
            if (up == null)
            {
        ...
        public bool RemoveDown(int id)
        {
            ...
            int orderValue = (int)current.Task_Tax; 
            //下一个对象，即弟弟对象；
            Task down = Gateway.Default.From<Task>().Where(Task._.Task_Tax <orderValue && Task._.Task_Level == current.Task_Level).OrderBy(Task._.Task_Tax.Desc).ToFirst<Task>();
            if (down == null)
            {
