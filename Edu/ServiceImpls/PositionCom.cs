...
namespace ServiceImpls
{
    public class PositionCom :IPosition
    {
        ...
        {
            Position entity = this.GetSingle(identify);
            this.Delete(entity);            
        }
        /// <summary>
        /// 删除，按职位名称
        ...
        {
            Position entity = this.GetSingle(orgid,name);
            this.Delete(entity); 
        }
        /// <summary>
        /// 删除与员工之间的关联
        ...
        {
            Organization org = Gateway.Default.From<Organization>().Where(Organization._.Org_IsRoot == true).ToFirst<Organization>();
        ...
        public Position GetAdmin(int orgid)
        {
            ...
            if (org == null) throw new Common.ExceptionForWarning("ID为" + orgid + "的机构不存在！");
