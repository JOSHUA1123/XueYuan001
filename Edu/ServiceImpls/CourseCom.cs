    ...
    public class CourseCom : ICourse
    {
        ...
        public void CourseAdd(Course entity)
        {
            ...
            if (string.IsNullOrWhiteSpace(entity.Cou_UID))
                entity.Cou_UID = Common.Request.UniqueID();
        ...
        public Course CourseBatchAdd(int orgid, int sbjid, string names)
        {
            ...
            {
                Course current = CourseIsExist(orgid, sbjid, pid, listName[i]);
                if (current == null)
                {
                    current = new Course();
                    ...
                    {
                        Teacher th = Extend.LoginState.Accounts.Teacher;
                        if (th != null)
                        {
        ...
        public void CourseDelete(Course entity)
        {
            ...
            Outline[] oul = Business.Do<IOutline>().OutlineAll(entity.Cou_ID, null);
            GuideColumns[] gcs = Business.Do<IGuide>().GetColumnsAll(entity.Cou_ID, null);
            using (DbTrans tran = Gateway.Default.BeginTrans())
            {
                try
                {
                    ...
                    foreach (Outline ac in oul)
                    {
                        Business.Do<IOutline>().OutlineDelete(ac);
                    }
                    foreach (GuideColumns gc in gcs)
                    {
        ...
        {
            Course ol = this.CourseSingle(identify);
        ...
        /// 获取单一实体对象，按课程名称；
        /// </summary>
        /// <param name="couName">课程名称</param>
        /// <returns></returns>
        public Course CourseSingle(string couName)
        {
            return Gateway.Default.From<Course>().Where(Course._.Cou_Name == couName).ToFirst<Course>();
        }
        /// <summary>
        ...
        public string CourseName(int identify)
        {
            ...
            Course tm = Gateway.Default.From<Course>().Where(Course._.Cou_ID == entity.Cou_PID).ToFirst<Course>();
            while (tm != null)
            {
        ...
        public Course IsBuyCourse(int couid, int stid, int state)
        {
            ...
            return Gateway.Default.From<Course>()
                    .InnerJoin<Student_Course>("[Student_Course].[Cou_ID] = [Course].[Cou_ID]")
        ...
        {
            //删除章节
            List<Outline> outline = Gateway.Default.From<Outline>().Where(Outline._.Cou_ID == identify).ToList<Outline>();
            ...
            //删除试卷
            List<TestPaper> tps = Gateway.Default.From<TestPaper>().Where(TestPaper._.Cou_ID == identify).ToList<TestPaper>();
            if (tps != null && tps.Count > 0)
            {
                foreach (TestPaper t in tps)
                    Business.Do<ITestPaper>().PagerDelete(t.Tp_Id);
            }
            //考试指南
            List<GuideColumns> gcs = Gateway.Default.From<GuideColumns>().Where(GuideColumns._.Cou_ID == identify).ToList<GuideColumns>();
            if (gcs != null && gcs.Count > 0)
            {
                foreach (GuideColumns t in gcs)
                    Business.Do<IGuide>().ColumnsDelete(t);
            }
            //清理试题
            List<Questions> ques = Gateway.Default.From<Questions>().Where(Questions._.Cou_ID == identify).ToList<Questions>();
            if (ques != null && ques.Count > 0)
            {
                foreach (Questions c in ques)
        ...
        /// <returns></returns>
        private int _ClacLevel(Course entity)
        {
            ...
            Gateway.Default.Save<Course>(entity);
            List<Course> childs = Gateway.Default.From<Course>().Where(Course._.Cou_PID == entity.Cou_ID).ToList<Course>();
        ...
        /// <param name="entity"></param>
        /// <returns></returns>
        private string _ClacXPath(Course entity)
        {
        ...
        {
            Student_Course sc = Gateway.Default.From<Student_Course>()
        ...
        /// <returns></returns>
        public Student_Course Buy(int stid, int couid, CoursePrice price)
        {
        ...
        public Student_Course Tryout(int stid, int couid)
        {
            //生成学员与课程的关联
            Student_Course sc = Business.Do<ICourse>().StudentCourse(stid, couid);
            if (sc == null) sc = new Student_Course();
            sc.Cou_ID = couid;
            sc.Ac_ID = stid;
            sc.Stc_StartTime = DateTime.Now;
        ...
        public List<Course> Course4Teacher(int thid, int count)
        {
            count = count < 1 ? int.MaxValue : count;
            return Gateway.Default.From<Course>()
                    .InnerJoin<Teacher_Course>("[Teacher_Course].[Cou_ID] = [Course].[Cou_ID]")
                    .Where(Teacher_Course._.Th_ID == thid)
                    .OrderBy(Course._.Cou_Tax.Desc).ToList<Course>(count);
        }
        /// <summary>
        ...
        public Accounts[] Student4Course(int couid, string stname, string stmobi, int size, int index, out int countSum)
        {
            ...
            countSum = Gateway.Default.From<Accounts>().InnerJoin<Student_Course>("[Student_Course].[Ac_ID] = [Accounts].[Ac_ID]")
                   .Where(wc).OrderBy(Accounts._.Ac_LastTime.Desc).Count();

            return Gateway.Default.From<Accounts>()
                   .InnerJoin<Student_Course>("[Student_Course].[Ac_ID] = [Accounts].[Ac_ID]")
        ...
        public void PriceSetCourse(string uid){           
            ...
            if (prices.Length > 0)
            {
                ...
                Course course = Gateway.Default.From<Course>().Where(Course._.Cou_UID == uid).ToFirst<Course>();
                if (course != null)
                {
