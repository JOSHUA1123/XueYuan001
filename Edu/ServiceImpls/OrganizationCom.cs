...
namespace ServiceImpls
{
    public class OrganizationCom : IOrganization
    {
        ...
        public void OrganAdd(Organization entity)
        {
            ...
            //解析地址
            Common.Param.Method.Position posi = Common.Request.Position(entity.Org_Longitude, entity.Org_Latitude);
        ...
        {           
            ...
            //从缓存中读取
            List<Organization> list = Common.Cache<Organization>.Data.List;
        ...
        public Organization OrganCurrent()
        {
            ...
            {
                //当前机构的二级域名
                string twoDomain = Common.Request.Domain.TwoDomain;
                List<Organization> tm = (from l in list
        ...
        public void OrganBuildQrCode()
        {
            this.OrganBuildCache();  //重新构建缓存  
            //批量生成二维码
            List<Organization> orgs = Common.Cache<Organization>.Data.List;
            if (orgs != null)
            {
        ...
        public void OrganBuildQrCode(Organization entity)
        {
            ...
            {
                string domain = Common.Server.MainName;
                string port = Common.Server.Port;
                url = "http://" + entity.Org_TwoDomain + "." + domain + ":" + port + "/Mobile/default.ashx";
            } 
            //各项配置           
            Common.CustomConfig config = CustomConfig.Load(entity.Org_Config);   //自定义配置项           
            ...
            if (isQrcenter && System.IO.File.Exists(centerImg))
                image = Common.QrcodeHepler.Encode(url, 200, centerImg, color, "#fff");           
            else            
                image = Common.QrcodeHepler.Encode(url, 200, color, "#fff");
            //将image转为base64
            string base64 = Common.Images.ImageTo.ToBase64(image);
        ...
        {
            string tmpPath = Common.Upload.Get["Temp"].Physics;
        ...
        public List<Organization> OrganBuildCache()
        {
            lock (lock_cache_build)
            {
                try
                {
                    Common.Cache<Organization>.Data.Clear();
                }
                catch
                {
                }
                ...
                {
                    Organization[] org = Gateway.Default.From<Organization>()
                        .OrderBy(Organization._.Org_RegTime.Desc).ToArray<Organization>();
                    Common.Cache<Organization>.Data.Fill(org);
                }
                return Common.Cache<Organization>.Data.List;
            }
        }
