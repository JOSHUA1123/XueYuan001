...
namespace ServiceImpls
{
    public class SMSCom : ISMS
    {
        ...
        {           
            ...
            SMS.ISMS sms = SMS.Gatway.GetService(smsCurr);
            sms.Current.User = Business.Do<ISystemPara>().GetValue(smsCurr + "SmsAcc");           
            //生成短信内容
            string rnd = Common.Request.Random(6, 1);    //验证码，随机四位数字
            ...
            try
            {
                ...
                SMS.SmsState state = sms.Send(phone, msg);
                ...
                if (state.Success)
                {
                    ...
                    //将随机字符转用ＭＤ５加密
                    cookie.Value = new Common.Param.Method.ConvertToAnyValue(rnd).MD5;
