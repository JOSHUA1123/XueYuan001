...
// Type: com.google.zxing.pdf417.decoder.BitMatrixParser
...
{
  /// <summary>
  /// <p>This class parses the BitMatrix image into codewords.
  ///             </p>
  /// </summary>
  ...
  internal sealed class BitMatrixParser
  {
    ...
    /// 
    /// </summary>
    private static readonly int[] SYMBOL_TABLE = new int[2787]
    {
      66142,
      66170,
      66206,
    ...
    internal int[] readCodewords()
    {
      ...
      int num1 = dimension;
      this.erasures = new int[929];
      float num2 = 1f;
      int[] rowCounters = new int[dimension];
      int[] numArray = new int[929];
      int num3 = 0;
      int num4 = 0;
      bool flag = false;
      int rowNumber = 0;
      ...
      for (int y = 1; y < num1; ++y)
      {
        ...
        {
          if (this.bitMatrix.get_Renamed(x, y) != this.bitMatrix.get_Renamed(x, y - 1))
            ++num5;
        }
        if ((double) num5 <= (double) num2 * 6.0)
        {
          for (int x = 0; x < dimension; ++x)
          {
            if (this.bitMatrix.get_Renamed(x, y))
              ++rowCounters[x];
          }
          ++num4;
          if ((double) num4 >= (double) num2 * 2.0)
            flag = true;
        }
        else
        {
          if (flag)
          {
            num3 = this.processRow(rowCounters, rowNumber, rowHeight, numArray, num3);
            if (num3 == -1)
              return (int[]) null;
            for (int index = 0; index < rowCounters.Length; ++index)
              rowCounters[index] = 0;
            ++rowNumber;
            rowHeight = 0;
          }
      ...
      this.erasures = BitMatrixParser.trimArray(this.erasures, this.eraseCount);
      return BitMatrixParser.trimArray(numArray, num3);
    }
    ...
    ///             </param><param name="size">the size to trim it to
    ///             </param>
    /// <returns>
    /// the new trimmed array
    /// 
    /// </returns>
    ...
    ///             in the row.
    ///             </param><param name="rowNumber">the current row number of codewords.
    ///             </param><param name="rowHeight">the height of this row in pixels.
    ///             </param><param name="codewords">the codeword array to save codewords into.
    ///             </param><param name="next">the next available index into the codewords array.
    ///             </param>
    /// <returns>
    /// the next available index into the codeword array after processing
    ///             this row.
    /// 
    /// </returns>
    internal int processRow(int[] rowCounters, int rowNumber, int rowHeight, int[] codewords, int next)
    {
      ...
      long symbol = 0L;
      int num2 = 0;
      while (num2 < dimension)
      {
        for (int index = 16; index >= 0; --index)
        {
          if (rowCounters[num2 + (16 - index)] >= SupportClass.URShift(rowHeight, 1))
            symbol |= 1L << index;
        }
        if (num1 > 0)
        {
          int codeword = BitMatrixParser.getCodeword(symbol);
          if (codeword < 0 && num2 < dimension - 17)
          {
            this.erasures[this.eraseCount] = next;
            ++next;
            ++this.eraseCount;
          }
          else
            codewords[next++] = codeword;
        }
        else
        {
          int codeword = BitMatrixParser.getCodeword(symbol);
          if (this.ecLevel < 0)
          {
            ...
            {
              case 1:
                this.leftColumnECData = codeword;
                break;
            }
          }
        }
      ...
      if (num1 > 1)
      {
        ...
        if (this.ecLevel < 0)
        {
          ...
          {
            case 2:
              this.rightColumnECData = codewords[next];
              if (this.rightColumnECData == this.leftColumnECData && this.leftColumnECData != 0)
              {
                this.ecLevel = (this.rightColumnECData % 30 - this.rows % 3) / 3;
                break;
              }
              break;
          }
    ...
    {
      int codewordIndex = BitMatrixParser.findCodewordIndex(symbol & 262143L);
      if (codewordIndex == -1)
        return -1;
      return (int) ((long) (BitMatrixParser.CODEWORD_TABLE[codewordIndex] - 1) % 929L);
    }
    ...
    ///             </param>
    /// <returns>
    /// the index into the codeword table.
    /// 
    /// </returns>
    private static int findCodewordIndex(long symbol)
    {
      int num1 = 0;
      int num2 = BitMatrixParser.SYMBOL_TABLE.Length;
      while (num1 < num2)
      {
        int index = SupportClass.URShift(num1 + num2, 1);
        if (symbol < (long) BitMatrixParser.SYMBOL_TABLE[index])
        {
          num2 = index;
        }
        else
