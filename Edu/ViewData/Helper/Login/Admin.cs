...
namespace ViewData.Helper
{
    ...
    public class Admin : IRequiresSessionState
    {
        ...
        private List<EntitiesInfo.EmpAccount> _onlineUser = new List<EmpAccount>();
        /// <summary>
        /// 在线用户
        /// </summary>
        public List<EmpAccount> OnlineUser
        {
        ...
        public string Value
        {
            get
            {
                ...
                if (Admin.LoginPattern == LoginPatternEnum.Cookies)
                    encrypt = Common.Request.Cookies[key].String;
                if (Admin.LoginPattern == LoginPatternEnum.Session)
                    encrypt = Common.Request.Session[key].String;
                return encrypt;
            }            
        }
        /// <summary>
        ...
        public int Expires
        {
            get
            {
                ...
                if (exp.Equals("auto", StringComparison.CurrentCultureIgnoreCase))
                    return int.MaxValue;
                return Common.Login.Get["Admin"].Expires.Int32 ?? 10;
            }
        }
        /// <summary>
        /// 当前登录用户的id
        ...
        /// </summary>
        /// <returns></returns>
        public EmpAccount CurrentUser
        {
            get
            {
                int userid = this.UserID;
                EmpAccount curr = null;
                //首先从在线列表中取当前登录的用户
        ...
        private string EncryptForDES(int id)
        {
            ...
            string twoDomain = Common.Request.Domain.TwoDomain;
            return Common.DataConvert.EncryptForDES(str, twoDomain); 
        }
        private string[] DecryptForDES(string encrypt)
        {
            string twoDomain = Common.Request.Domain.TwoDomain;
            string str = Common.DataConvert.DecryptForDES(encrypt, twoDomain);
        ...
        public void Write(EmpAccount acc)
        {
            string domain = Common.Request.Domain.TwoDomain;
            Write(acc, domain);
        }
        ...
        public void Write(EmpAccount acc, string twoDomain)
        {
        ...
        /// </summary>
        /// <param name="acc"></param>
        private void Register(EmpAccount acc)
        {
            if (acc == null) return;
        ...
        public void CleanOut()
        {
            //设置超时时间，单位分钟
            int outTimeNumer = 3;
            List<EmpAccount> _tm = new List<EmpAccount>();
            foreach (EmpAccount em in this.OnlineUser)
            {
                if (em == null) continue;
