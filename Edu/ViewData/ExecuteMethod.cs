        ...
        public static DataResult ExecToResult(Letter letter)
        {
            try
            {
                if (!"weishakeji".Equals(letter.HTTP_Mark))
                    return new ViewData.DataResult(new Exception("请求标识不正确"));

                object res = Exec(letter);
                if (res is ListResult) return (ListResult)res;  //如果是分页数据
                return new ViewData.DataResult(res);       //普通数据
            }
            catch (Exception ex)
            {
                return new ViewData.DataResult(ex);
            }
        }
        /// <summary>
        /// 要执行的方法，根据方法名、参数数量
