...
{
    /// <summary>
    /// 当客户端请求ViewData方法时，返回值全部用该方法“包装”，方便返回json或xml格式数据
    /// 如果是服务器端方法，则无须“包装”
    /// </summary>
        ...
        private string _json_property(string typename, object value)
        {
            ...
            switch (typename)
            {
                ...
                case "Boolean":
                    ...
                    break;
                case "Object":
                    if (value is DataBaseInfo.Entity)
                    {
                        str = ((DataBaseInfo.Entity)value).ToJson();
                    }
                    else
                    {
                        if (value is DataBaseInfo.Entity[])
                        {
                            DataBaseInfo.Entity[] entitis = (DataBaseInfo.Entity[])value;
                            str += "[";
                            foreach (DataBaseInfo.Entity en in entitis)
                                str += en.ToJson() + ",";
                            if (str.EndsWith(",")) str = str.Substring(0, str.Length - 1);
                            str += "]";
                        }
        ...
        private string _xml_property(string typename, object value)
        {
            ...
            switch (typename)
            {
                ...
                default:
                    ...
                    if (value is DataBaseInfo.Entity)
                    {
                        str = ((DataBaseInfo.Entity)value).ToXML();
                    }
                    else
                    {
                        if (value is DataBaseInfo.Entity[])
