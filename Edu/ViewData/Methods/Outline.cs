...
namespace ViewData.Methods
{
    ...
    public class Outline : IViewAPI
    {
        ...
        /// <returns></returns>
        [HttpGet]
        public EntitiesInfo.Outline ForID(int id)
        {
        ...
        /// <returns></returns>
        [HttpGet(Ignore = true)]
        public EntitiesInfo.Outline[] List(int couid, int pid)
        {
        ...
        {
            // 当前课程的所有章节            
            EntitiesInfo.Outline[] outlines = Business.Do<IOutline>().OutlineAll(couid, true);
            if (outlines.Length > 0)
            {
                foreach (EntitiesInfo.Outline ol in outlines)
                {
        ...
        /// <returns></returns>
        public List<EntitiesInfo.Accessory> Accessory(string uid)
        {
            //先判断是否购买课程
            EntitiesInfo.Accounts acc = Extend.LoginState.Accounts.CurrentUser;
            if (acc == null) return new List<Accessory>();
            EntitiesInfo.Outline outline = Business.Do<IOutline>().OutlineSingle(uid);
            EntitiesInfo.Course course = Business.Do<ICourse>().CourseSingle(outline.Cou_ID);
            //是否免费，或是限时免费
            ...
            //获取附件
            List<EntitiesInfo.Accessory> access = Business.Do<IAccessory>().GetAll(uid, "Course");
        ...
        public Dictionary<string, object> State(int olid)
        {
            ...
            EntitiesInfo.Outline outline = Business.Do<IOutline>().OutlineSingle(olid);
            ...
            if (acc != null)
            {
                ...
                //学习记录
                EntitiesInfo.LogForStudentStudy studyLog = Business.Do<IStudent>().LogForStudySingle(acc.Ac_ID, outline.Ol_ID);
            ...
            List<EntitiesInfo.Accessory> videos = Business.Do<IAccessory>().GetAll(outline.Ol_UID, "CourseVideo");
            ...
            if (videos.Count > 0)
            {
                ...
                if (existVideo && !videos[0].As_IsOuter)
                {
                    ...
                    string fileHy = Common.Server.MapPath(videoUrl);
