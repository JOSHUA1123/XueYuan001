...
namespace ViewData.Methods
{
    ...
    public class Teacher : IViewAPI
    {
        ...
        /// <returns></returns>
        public EntitiesInfo.Teacher Login(string acc,string pw)
        {
            EntitiesInfo.Accounts account = Business.Do<IAccounts>().AccountsLogin(acc, pw, true);
            if (account == null) return null;
            if (!account.Ac_IsTeacher) return null;
            //
            EntitiesInfo.Teacher teacher = Business.Do<ITeacher>().TeacherSingle(account.Ac_AccName, -1);
            if (teacher != null)
            {
                teacher.Th_Photo = Common.Upload.Get["Accounts"].Virtual + account.Ac_Photo;
        ...
        /// <returns></returns>
        public EntitiesInfo.Course[] Courses(int thid)
        {
            List<EntitiesInfo.Course> cours = Business.Do<ICourse>().CourseAll(-1, -1, thid, true);
            if (cours == null || cours.Count < 1) return null;
            return cours.ToArray<EntitiesInfo.Course>();
        }
        /// <summary>
        /// 教师的直播课
        /// </summary>
        ...
        /// <returns></returns>
        public EntitiesInfo.Course[] LiveCourses(int thid, int count)
        {
            List<EntitiesInfo.Course> cours = Business.Do<ICourse>().CourseCount(-1, -1, thid, true, null, true, count);
            if (cours == null || cours.Count < 1) return null;
            return cours.ToArray<EntitiesInfo.Course>();
        }
        /// <summary>
        ...
        public Outline_LiveInfo[] Lives(int thid, int couid)
        {
            ...
            //
            EntitiesInfo.Outline[] outls = Business.Do<IOutline>().OutlineCount(couid, true, null, true, 0);
            ...
            foreach (EntitiesInfo.Outline o in outls)
            {
