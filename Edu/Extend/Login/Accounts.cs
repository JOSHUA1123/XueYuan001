...
namespace Extend.Login
{
    public class Accounts
    {
        ...
        private Accounts(){
            ...
            string exp = Common.Login.Get["Accounts"].Expires.String;
            if (!exp.Equals("auto", StringComparison.CurrentCultureIgnoreCase))
                Expires = Common.Login.Get["Accounts"].Expires.Int32 ?? 10;
            // 后台管理登录的状态管理方式，值为Cookies或Session
            string tm = Common.Login.Get["Accounts"].Pattern.String;
            ...
            //登录标识名
            this.KeyName = Common.Login.Get["Accounts"].KeyName.String;   
        }
        #endregion
        ...
        /// 在线学员
        /// </summary>
        public List<EntitiesInfo.Accounts> OnlineUser
        {
            get
            {
                lock (_lock)
                {
                    List<EntitiesInfo.Accounts> list = HttpRuntime.Cache.Get("AccountsCache") as List<EntitiesInfo.Accounts>;
                    if (list == null) list = new List<EntitiesInfo.Accounts>();
                    return list;
                }
            }
            set
        ...
        public string UID
        {
            get { return Common.Request.Cookies["AccountsUID"].String; }
            set
            {
        ...
        public void OnlineUserAdd(EntitiesInfo.Accounts acc)
        {
            List<EntitiesInfo.Accounts> list = this.OnlineUser;
            list.Add(acc);
        ...
        /// </summary>
        /// <returns></returns>
        public EntitiesInfo.Accounts CurrentUser
        {
            get
            {
                ...
                EntitiesInfo.Accounts curr = null; //当前用户
                //首先从在线列表中取当前登录的用户
                List<EntitiesInfo.Accounts> list = this.OnlineUser;
                if (list.Count > 0)
                {
                    ...
                    {
                        EntitiesInfo.Accounts em = list[i];
                        if (em.Ac_ID == acid)
                        {
                            //如果不是在微信中，同一账号不能同时登录
                            if (!Common.Browser.IsWeixin && em.Ac_CheckUID == uid)
                                curr = em;
                            break;
                        }
                    }                    
        ...
        /// 当前登录的教师信息
        /// </summary>
        public EntitiesInfo.Teacher Teacher
        {
            get
            {
                int acid = this.UserID;  //当前用户的账号id
                if (acid <= 0) return null;
                EntitiesInfo.Teacher th = Business.Do<IAccounts>().GetTeacher(acid, true);
                return th;
            }
        }
        /// <summary>
        ...
        public int UserID
        {
            get
            {
                ...
                if (this.LoginPattern == LoginPatternEnum.Cookies)
                    accid = Common.Request.Cookies[KeyName].Int32 ?? 0;
                if (this.LoginPattern == LoginPatternEnum.Session)
                    accid = Common.Request.Session[KeyName].Int32 ?? 0;
                return accid;
            }
        }
        /// <summary>
        ...
        /// </summary>
        /// <param name="acc"></param>
        public void Write(EntitiesInfo.Accounts acc)
        {
            this.Write(acc, -1);
        }
        /// <summary>
        ...
        public void Write(EntitiesInfo.Accounts acc, int expiresDay)
        {
            System.Web.HttpContext _context = System.Web.HttpContext.Current;
            if (this.LoginPattern == LoginPatternEnum.Cookies)
            {
        ...
        /// </summary>
        /// <param name="acc"></param>
        private void _register(EntitiesInfo.Accounts acc)
        {
            if (acc == null) return;
            //登录时间，该时间不入数据库，仅为临时使用
            acc.Ac_LastTime = DateTime.Now;
            acc.Ac_LastIP = Common.Browser.IP;   //写入登录时的IP
            //写入唯一值，用于判断同一个账号是否多人登录
            acc.Ac_CheckUID = Common.Request.UniqueID();
            ...
            Business.Do<IAccounts>().AccountsUpdate(acc,
                new Field[] { EntitiesInfo.Accounts._.Ac_LastTime, EntitiesInfo.Accounts._.Ac_LastIP, EntitiesInfo.Accounts._.Ac_CheckUID },
        ...
        public void Refresh(EntitiesInfo.Accounts st)
        {
            if (st == null) return;
            List<EntitiesInfo.Accounts> list = this.OnlineUser;
            lock (_lock)
        ...
        public void CleanOut(EntitiesInfo.Accounts acc)
        {
        ...
        public void VerifyLogin()
        {
            ...
            if (path != "index.aspx")
            {
                ...
                {
                    //如果未登录
                    string url = Common.Login.Get["Accounts"].NoLoginPath.String ?? "/";
        ...
        public void Course(EntitiesInfo.Course cou)
        {
            //记录到数据库
            EntitiesInfo.Accounts st = this.CurrentUser;
            if (st != null)
            {
        ...
        /// </summary>
        /// <returns></returns>
        public EntitiesInfo.Course Course()
        {
            EntitiesInfo.Course c = null;
            EntitiesInfo.Accounts st = this.CurrentUser;
