...
namespace Extend
{
    ...
    public class QrCode
    {
        ...
        /// <returns></returns>
        public static string Creat4Entity(DataBaseInfo.Entity entity, string template, string qrcodeImgPath, int wh)
        {
            ...
            if (isCenterImg)
            {
                ...
                image = Common.QrcodeHepler.Encode(template, wh, centerImg, color, null);
            }
            else
            {
                image = Common.QrcodeHepler.Encode(template, wh, color, null);
            }
        ...
        {
            Organization org = Business.Do<IOrganization>().OrganDefault();
            //企业网站域名
