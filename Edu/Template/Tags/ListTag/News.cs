...
namespace Template.Tags.ListTag
{
    ...
    public class News : TagElement
    {
        public override void DataBind()
        {
            ...
            if (from == null)
            {
                EntitiesInfo.Article[] news = Business.Do<IContents>().ArticleCount(this.Organ.Org_ID, columns, count, order);
                foreach (EntitiesInfo.Article n in news)
                {
                    n.Art_Logo = Upload.Get["News"].Virtual + n.Art_Logo;
                    if (string.IsNullOrWhiteSpace(n.Art_Intro))
                    {
                        n.Art_Intro = Common.HTML.ClearTag(n.Art_Details, 200);
                    }
                }
                //指定数据源
                tag.DataSourse = news;
