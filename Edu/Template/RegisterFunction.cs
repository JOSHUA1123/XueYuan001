...
namespace Template
{
    ...
    public class RegisterFunction
    {
        ...
        protected static string ClearHtml(object[] p)
        {
            ...
            return Common.HTML.ClearTag(html);
        }
        /// <summary>
        ...
        /// <returns></returns>
        protected static List<EntitiesInfo.Course> getCourses(object[] para)
        {
            EntitiesInfo.Organization org = Business.Do<IOrganization>().OrganCurrent();
            //专业id
            int sbj = 0, count = 0;
            ...
            List<EntitiesInfo.Course> cour = Business.Do<ICourse>().CourseCount(org.Org_ID, sbj, null, true, order, count);
        ...
        /// <returns></returns>
        protected static EntitiesInfo.CoursePrice[] getCoursePrice(object[] para)
        {            
            //课程uid
            string couuid = null;
            if (para.Length > 0 && para[0] != null) couuid = para[0].ToString();
            if (string.IsNullOrWhiteSpace(couuid)) return null;
            EntitiesInfo.CoursePrice[] prices = Business.Do<ICourse>().PriceCount(0, couuid, true, 0);
            return prices;
        }
        /// <summary>
        /// 获取新闻
        ...
        /// <returns></returns>
        protected static EntitiesInfo.Article[] getArticle(object[] para)
        {
            EntitiesInfo.Organization org = Business.Do<IOrganization>().OrganCurrent();
            int col = 0, count = 0;
            if (para.Length > 0 && para[0] is int)
            ...
            if (para.Length > 1 && para[1] is int)
                int.TryParse(para[1].ToString(), out count);
            EntitiesInfo.Article[] arts = Business.Do<IContents>().ArticleCount(org.Org_ID, col, count, "hot");
            foreach (EntitiesInfo.Article c in arts)
        ...
        /// </summary>
        /// <returns></returns>
        protected static EntitiesInfo.Links[] getLinks(object[] paras)
        {
            EntitiesInfo.Organization org = Business.Do<IOrganization>().OrganCurrent();
